...
describe('post144 parser HEAVY deepen (after #144)', () => {
  const root = parserRoot;
  const read = (rel: string) => readFileSync(join(root, rel), 'utf8');
  const sha256 = (rel: string) => createHash('sha256').update(readFileSync(join(root, rel))).digest('hex');
  const sha1 = (rel: string) => createHash('sha1').update(readFileSync(join(root, rel))).digest('hex');
  ...
  it('post144: parseM3U skips rtmp and dedupes urls', () => {
    const m3u = buildSimpleM3U([
      { name: 'A', url: 'https://example.com/a.m3u8' },
      { name: 'B', url: 'https://example.com/a.m3u8' },
      { name: 'C', url: 'rtmp://example.com/live' },
    ]);
    // rtmp line won't be emitted by buildSimpleM3U as http — inject raw
    const raw = '#EXTM3U\n#EXTINF:-1 tvg-name="A",A\nhttps://example.com/a.m3u8\n#EXTINF:-1 tvg-name="B",B\nhttps://example.com/a.m3u8\n#EXTINF:-1 tvg-name="C",C\nrtmp://example.com/live\n';
    const stations = parseM3U(raw);
    expect(stations).toHaveLength(1);
    expect(stations[0].name).toBe('A');
  ...
  it('post144: parseM3U trims and keeps http', () => {
    const raw = '#EXTM3U\n#EXTINF:-1 tvg-name=" News ", News \nhttp://example.com/n.m3u8\n';
    const stations = parseM3U(raw);
    expect(stations).toHaveLength(1);
    expect(stations[0].url).toBe('http://example.com/n.m3u8');
  });

  it('post144: SAMPLE_M3U countHttpStreamLines 6', () => {
    expect(countHttpStreamLines(SAMPLE_M3U)).toBe(6);
  });
  ...
  it('post144: final inventory markers', () => {
    const body = read('test/parser.test.ts');
    expect(body).toContain("describe('post144 parser HEAVY deepen (after #144)'");
    expect((body.match(/it\('post144:/g) ?? []).length).toBeGreaterThan(60);
  });
});


describe('post144 parser extras HEAVY deepen (after #144 leftover slice)', () => {
  const root = parserRoot;
  const read = (rel: string) => readFileSync(join(root, rel), 'utf8');
  const sha256 = (rel: string) => createHash('sha256').update(readFileSync(join(root, rel))).digest('hex');
  const sha1 = (rel: string) => createHash('sha1').update(readFileSync(join(root, rel))).digest('hex');
  ...
  it('post144-extras: inventory digest', () => {
    const keys = ["post144","leftover","TOKENMAXX","HEAVY","after-#144","no-product-invent","slice-diff","no-src-change","manifest-lock","HITL","parser","extras","VALID_GENRES","GENRE_MAP","CATALOG_CACHE","post141"];
    expect(createHash('sha256').update(keys.join('|'), 'utf8').digest('hex')).toBe('6e7142df66e8faba638cb3ceeb257645fd19f453e00157f3887adc4c281d61e6');
  ...
  it('post144-extras: final markers', () => {
    const body = read('test/parser.test.ts');
    expect(body).toContain("describe('post144 parser HEAVY deepen (after #144)'");
    expect(body).toContain("describe('post144 parser extras HEAVY deepen (after #144 leftover slice)'");
