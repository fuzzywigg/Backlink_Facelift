...
// Deepens existing parser score/rank invent fences, missing meta, duplicate canonical stubs, zero-link pages.
// No product inventing — tests/CI only.

describe('overnight-seo parser HEAVY deepen (seo-score-validators after #149)', () => {
  const root = parserRoot;
  const read = (rel: string) => readFileSync(join(root, rel), 'utf8');
  const sha256 = (rel: string) => createHash('sha256').update(readFileSync(join(root, rel))).digest('hex');
  const sha1 = (rel: string) => createHash('sha1').update(readFileSync(join(root, rel))).digest('hex');
  ...
  it('overnight-seo: locks test/helpers.ts sha256', () => {
    expect(sha256('test/helpers.ts')).toBe('240e1fc521e029b07ca3ebda83410c4a4014af02f3ad64fa4eba8bf6ffd3af29');
  });
  it('overnight-seo: locks test/helpers.ts sha1', () => {
    expect(sha1('test/helpers.ts')).toBe('aac5e2154aa8f0784db092ad4bb51304fce6e117');
  });
  it('overnight-seo: locks test/helpers.ts md5', () => {
    expect(md5('test/helpers.ts')).toBe('004bbc8741017d8dd45bee28a29b46e1');
  });
  it('overnight-seo: locks test/helpers.ts sha384', () => {
    expect(sha384('test/helpers.ts')).toBe('1e769f73400f921f25168ef2d408d099e12eee86ee092cf9883c0fe30149a90772171be2e8a13ac92b09294194f38167');
  });
  it('overnight-seo: locks test/helpers.ts sha512', () => {
    expect(sha512('test/helpers.ts')).toBe('153eabb426836a56130b49b90611260d3630cf906663d61e1c0c6752819c3907b8dfbf9cc88531336b9a04c9d1c60b95a81d0e7ee97418122915c87377ff2c91');
  });
  it('overnight-seo: locks test/helpers.ts sha3-256', () => {
    expect(sha3('test/helpers.ts')).toBe('8ffbb4baecd580e1f9f797a737d24af1f3e0fb48af208435dafe8afaa584b113');
  });
  it('overnight-seo: locks test/helpers.ts blake2b512', () => {
    expect(blake2b('test/helpers.ts')).toBe('9000b1e34f31a60c5b766398de6ce5657f7325d791b129e388e1312c47d8448070919d7711a2824b821a99661fbeb73f6e13dfa4b1de53d72aa0990f73f1061f');
  });
  it('overnight-seo: locks test/helpers.ts ripemd160', () => {
    expect(ripemd('test/helpers.ts')).toBe('24c482ba1ff1b74537b67a89b99058b6f2e500a4');
  });
  it('overnight-seo: locks test/helpers.ts size 6078', () => {
    expect(statSync(join(root, 'test/helpers.ts')).size).toBe(6078);
    expect(readFileSync(join(root, 'test/helpers.ts')).byteLength).toBe(6078);
  });
  it('overnight-seo: locks test/helpers.ts utf8 6078 lines 164', () => {
    expect(read('test/helpers.ts')).toHaveLength(6078);
    expect(read('test/helpers.ts').split('\n')).toHaveLength(164);
  });
  it('overnight-seo: locks test/helpers.ts nibble 487 xor 5', () => {
    const d = sha256('test/helpers.ts');
    expect(nibbleSum(d)).toBe(487);
    expect(xorNibbles(d)).toBe(5);
  });
  it('overnight-seo: locks test/helpers.ts pairSum 4132 rollingXor 216', () => {
    const d = sha256('test/helpers.ts');
    expect(pairSum(d)).toBe(4132);
    expect(rollingXor(d)).toBe(216);
  });
  it('overnight-seo: locks test/helpers.ts first/last/mid octets', () => {
    const d = sha256('test/helpers.ts');
    expect(d.slice(0, 2)).toBe('24');
    expect(d.slice(-2)).toBe('29');
    expect(d.slice(28, 36)).toBe('0c4a4014');
  });
  it('overnight-seo: locks test/helpers.ts HMAC overnight/seo-score/validators', () => {
    expect(hmacSha256('overnight', 'test/helpers.ts')).toBe('693bcb8a40aedd80ecdbc898e9f2d3483937ef1791d2ced6b4c65dea5c3ac764');
    expect(hmacSha256('seo-score', 'test/helpers.ts')).toBe('e6ab9b0795b02a69b0c150cdee65873db7f7d407355673848d61cc282a606c59');
    expect(hmacSha256('validators', 'test/helpers.ts')).toBe('63f9f5364c5d2acc65bf72e82d1cd23061ac5993c4c86278edb9ce2b06a50a5d');
  });
  it('overnight-seo: locks test/helpers.ts HMAC TOKENMAXX/HEAVY/after-#149', () => {
    expect(hmacSha256('TOKENMAXX', 'test/helpers.ts')).toBe('8b1973547653b49511673307302184ed795b388e025a43d50e0b32fc3e476391');
    expect(hmacSha256('HEAVY', 'test/helpers.ts')).toBe('458cfb306ea3e2c9310b3e3840ecd5a5ca295e18c46146bad4c6111c4c3c1c24');
    expect(hmacSha256('after-#149', 'test/helpers.ts')).toBe('c11a1ddd4b43d46935bccba3c329ecfebf6bf71aaf50a172e7ba6d9bee92668f');
  });
  it('overnight-seo: locks test/helpers.ts HMAC leftover/no-product-invent/boundary-scores', () => {
    expect(hmacSha256('leftover', 'test/helpers.ts')).toBe('3cf47764c97927288d3627ddc71cecde2a6313dea10d1f5941a8cce945d8b3d1');
    expect(hmacSha256('no-product-invent', 'test/helpers.ts')).toBe('a6bca018ba16cafff8a3d108bbc4c96391d8da76ccdaf6927e095cbf2e763870');
    expect(hmacSha256('boundary-scores', 'test/helpers.ts')).toBe('2201727872076ba7d2d5521f4149ac758d112331d5f7eb3c90d69dd3bd4c17c5');
  });
  it('overnight-seo: locks test/helpers.ts HMAC missing-meta/duplicate-canonical/zero-link', () => {
    expect(hmacSha256('missing-meta', 'test/helpers.ts')).toBe('d9d0dfa651895add9210fb39b735e931f2b4f0dc5fe7b33e1fd961881d83e585');
    expect(hmacSha256('duplicate-canonical', 'test/helpers.ts')).toBe('129d7b91b363522372355951e067a8152a5c02e5367bac2745d472f8211079f3');
    expect(hmacSha256('zero-link', 'test/helpers.ts')).toBe('6aba915e6aeae2ea0aaaf956e55b654f9a7f53bd29f8827a2e05a9b06403f045');
  });
  it('overnight-seo: locks test/helpers.ts HMAC canonical-stub/seo-score-validators', () => {
    expect(hmacSha256('canonical-stub', 'test/helpers.ts')).toBe('e79434ea456f0f4bacc046ce8820e4c06679eb167c7d1ac8f879aab24b9facad');
    expect(hmacSha256('seo-score-validators', 'test/helpers.ts')).toBe('d4818a24c03ba7bfc37c9c7ab1614a1c4d7bcaa3fdc394b036bfe00cffd213d6');
  });
  it('overnight-seo: locks test/helpers.ts spaces 919', () => {
    expect((read('test/helpers.ts').match(/ /g) ?? []).length).toBe(919);
  });
  it('overnight-seo: locks test/helpers.ts reversed sha256', () => {
    const rev = [...read('test/helpers.ts')].reverse().join('');
    expect(createHash('sha256').update(rev).digest('hex')).toBe('e4e0c03b4add5880ffd0e08d925c61f9b010f475d513abd32e2baa8f79ef7625');
  });
  it('overnight-seo: locks test/helpers.ts sha256 UPPERCASE', () => {
    expect(sha256('test/helpers.ts').toUpperCase()).toBe('240E1FC521E029B07CA3EBDA83410C4A4014AF02F3AD64FA4EBA8BF6FFD3AF29');
  });
  it('overnight-seo: locks test/helpers.ts first-line sha256', () => {
    expect(createHash('sha256').update(read('test/helpers.ts').split('\n')[0] ?? '').digest('hex')).toBe('e33b7958170700d0297aa0c9ad1db4689002e9bcee93e0e92b4e20e4837fbe22');
  });
  it('overnight-seo: locks test/helpers.ts size*lines 996792', () => {
    expect(statSync(join(root, 'test/helpers.ts')).size * read('test/helpers.ts').split('\n').length).toBe(996792);
  });
  it('overnight-seo: locks test/helpers.ts char-class tabs/nl/comma/colon/quotes', () => {
    const t = read('test/helpers.ts');
    expect((t.match(/\t/g) ?? []).length).toBe(0);
    expect((t.match(/\n/g) ?? []).length).toBe(163);
    expect((t.match(/,/g) ?? []).length).toBe(58);
  ...
  it('overnight-seo: locks test/helpers.ts HMAC-SHA1/MD5 key overnight-seo', () => {
    expect(createHmac('sha1', 'overnight-seo').update(readFileSync(join(root, 'test/helpers.ts'))).digest('hex')).toBe('8e33dd38b0eb30b6da79b7789da0969f7b8a73b9');
    expect(createHmac('md5', 'overnight-seo').update(readFileSync(join(root, 'test/helpers.ts'))).digest('hex')).toBe('6111017751e24990c240842cd56f65d2');
  });
  ...
  it('overnight-seo: boundary-scores does not invent station.score', () => {
    const [s] = parseM3U('#EXTM3U\n#EXTINF:-1 tvg-name="N" group-title="G",N\nhttps://seo.example/n\n');
    expect(s).not.toHaveProperty('score');
    expect(Object.keys(s).sort()).toEqual(['country', 'group', 'language', 'logo', 'name', 'url']);
  });
  it('overnight-seo: boundary-scores does not invent station.rank', () => {
    const [s] = parseM3U('#EXTM3U\n#EXTINF:-1 tvg-name="N" group-title="G",N\nhttps://seo.example/n\n');
    expect(s).not.toHaveProperty('rank');
    expect(Object.keys(s).sort()).toEqual(['country', 'group', 'language', 'logo', 'name', 'url']);
  });
  it('overnight-seo: boundary-scores does not invent station.priority', () => {
    const [s] = parseM3U('#EXTM3U\n#EXTINF:-1 tvg-name="N" group-title="G",N\nhttps://seo.example/n\n');
    expect(s).not.toHaveProperty('priority');
    expect(Object.keys(s).sort()).toEqual(['country', 'group', 'language', 'logo', 'name', 'url']);
  });
  it('overnight-seo: boundary-scores does not invent station.weight', () => {
    const [s] = parseM3U('#EXTM3U\n#EXTINF:-1 tvg-name="N" group-title="G",N\nhttps://seo.example/n\n');
    expect(s).not.toHaveProperty('weight');
    expect(Object.keys(s).sort()).toEqual(['country', 'group', 'language', 'logo', 'name', 'url']);
  });
  it('overnight-seo: boundary-scores does not invent station.seo', () => {
    const [s] = parseM3U('#EXTM3U\n#EXTINF:-1 tvg-name="N" group-title="G",N\nhttps://seo.example/n\n');
    expect(s).not.toHaveProperty('seo');
    expect(Object.keys(s).sort()).toEqual(['country', 'group', 'language', 'logo', 'name', 'url']);
  });
  it('overnight-seo: boundary-scores does not invent station.seoScore', () => {
    const [s] = parseM3U('#EXTM3U\n#EXTINF:-1 tvg-name="N" group-title="G",N\nhttps://seo.example/n\n');
    expect(s).not.toHaveProperty('seoScore');
    expect(Object.keys(s).sort()).toEqual(['country', 'group', 'language', 'logo', 'name', 'url']);
  });
  it('overnight-seo: boundary-scores does not invent station.pageRank', () => {
    const [s] = parseM3U('#EXTM3U\n#EXTINF:-1 tvg-name="N" group-title="G",N\nhttps://seo.example/n\n');
    expect(s).not.toHaveProperty('pageRank');
    expect(Object.keys(s).sort()).toEqual(['country', 'group', 'language', 'logo', 'name', 'url']);
  });
  it('overnight-seo: boundary-scores does not invent station.pagerank', () => {
    const [s] = parseM3U('#EXTM3U\n#EXTINF:-1 tvg-name="N" group-title="G",N\nhttps://seo.example/n\n');
  ...
  it('overnight-seo: boundary-scores does not invent station.authority', () => {
    const [s] = parseM3U('#EXTM3U\n#EXTINF:-1 tvg-name="N" group-title="G",N\nhttps://seo.example/n\n');
    expect(s).not.toHaveProperty('authority');
    expect(Object.keys(s).sort()).toEqual(['country', 'group', 'language', 'logo', 'name', 'url']);
  });
  it('overnight-seo: boundary-scores does not invent station.trust', () => {
    const [s] = parseM3U('#EXTM3U\n#EXTINF:-1 tvg-name="N" group-title="G",N\nhttps://seo.example/n\n');
    expect(s).not.toHaveProperty('trust');
    expect(Object.keys(s).sort()).toEqual(['country', 'group', 'language', 'logo', 'name', 'url']);
  });
  it('overnight-seo: boundary-scores does not invent station.domainAuthority', () => {
    const [s] = parseM3U('#EXTM3U\n#EXTINF:-1 tvg-name="N" group-title="G",N\nhttps://seo.example/n\n');
    expect(s).not.toHaveProperty('domainAuthority');
    expect(Object.keys(s).sort()).toEqual(['country', 'group', 'language', 'logo', 'name', 'url']);
  });
  it('overnight-seo: boundary-scores does not invent station.rating', () => {
    const [s] = parseM3U('#EXTM3U\n#EXTINF:-1 tvg-name="N" group-title="G",N\nhttps://seo.example/n\n');
    expect(s).not.toHaveProperty('rating');
    expect(Object.keys(s).sort()).toEqual(['country', 'group', 'language', 'logo', 'name', 'url']);
  });
  it('overnight-seo: boundary-scores does not invent station.stars', () => {
    const [s] = parseM3U('#EXTM3U\n#EXTINF:-1 tvg-name="N" group-title="G",N\nhttps://seo.example/n\n');
    expect(s).not.toHaveProperty('stars');
    expect(Object.keys(s).sort()).toEqual(['country', 'group', 'language', 'logo', 'name', 'url']);
  });
  it('overnight-seo: boundary-scores does not invent station.quality', () => {
    const [s] = parseM3U('#EXTM3U\n#EXTINF:-1 tvg-name="N" group-title="G",N\nhttps://seo.example/n\n');
    expect(s).not.toHaveProperty('quality');
    expect(Object.keys(s).sort()).toEqual(['country', 'group', 'language', 'logo', 'name', 'url']);
  });
  it('overnight-seo: boundary-scores does not invent station.relevance', () => {
    const [s] = parseM3U('#EXTM3U\n#EXTINF:-1 tvg-name="N" group-title="G",N\nhttps://seo.example/n\n');
    expect(s).not.toHaveProperty('relevance');
    expect(Object.keys(s).sort()).toEqual(['country', 'group', 'language', 'logo', 'name', 'url']);
  });
  it('overnight-seo: boundary-scores does not invent station.confidence', () => {
    const [s] = parseM3U('#EXTM3U\n#EXTINF:-1 tvg-name="N" group-title="G",N\nhttps://seo.example/n\n');
    expect(s).not.toHaveProperty('confidence');
    expect(Object.keys(s).sort()).toEqual(['country', 'group', 'language', 'logo', 'name', 'url']);
  });
  it('overnight-seo: boundary-scores does not invent station.probability', () => {
    const [s] = parseM3U('#EXTM3U\n#EXTINF:-1 tvg-name="N" group-title="G",N\nhttps://seo.example/n\n');
    expect(s).not.toHaveProperty('probability');
    expect(Object.keys(s).sort()).toEqual(['country', 'group', 'language', 'logo', 'name', 'url']);
  });
  it('overnight-seo: boundary-scores does not invent station.fitness', () => {
    const [s] = parseM3U('#EXTM3U\n#EXTINF:-1 tvg-name="N" group-title="G",N\nhttps://seo.example/n\n');
    expect(s).not.toHaveProperty('fitness');
    expect(Object.keys(s).sort()).toEqual(['country', 'group', 'language', 'logo', 'name', 'url']);
  });
  it('overnight-seo: boundary-scores does not invent station.boost', () => {
    const [s] = parseM3U('#EXTM3U\n#EXTINF:-1 tvg-name="N" group-title="G",N\nhttps://seo.example/n\n');
    expect(s).not.toHaveProperty('boost');
    expect(Object.keys(s).sort()).toEqual(['country', 'group', 'language', 'logo', 'name', 'url']);
  });
  it('overnight-seo: boundary-scores does not invent station.penalty', () => {
    const [s] = parseM3U('#EXTM3U\n#EXTINF:-1 tvg-name="N" group-title="G",N\nhttps://seo.example/n\n');
    expect(s).not.toHaveProperty('penalty');
    expect(Object.keys(s).sort()).toEqual(['country', 'group', 'language', 'logo', 'name', 'url']);
  });
  it('overnight-seo: boundary-scores does not invent station.canonical', () => {
    const [s] = parseM3U('#EXTM3U\n#EXTINF:-1 tvg-name="N" group-title="G",N\nhttps://seo.example/n\n');
    expect(s).not.toHaveProperty('canonical');
    expect(Object.keys(s).sort()).toEqual(['country', 'group', 'language', 'logo', 'name', 'url']);
  });
  it('overnight-seo: boundary-scores does not invent station.canonicalUrl', () => {
    const [s] = parseM3U('#EXTM3U\n#EXTINF:-1 tvg-name="N" group-title="G",N\nhttps://seo.example/n\n');
    expect(s).not.toHaveProperty('canonicalUrl');
    expect(Object.keys(s).sort()).toEqual(['country', 'group', 'language', 'logo', 'name', 'url']);
  });
  it('overnight-seo: boundary-scores does not invent station.hreflang', () => {
    const [s] = parseM3U('#EXTM3U\n#EXTINF:-1 tvg-name="N" group-title="G",N\nhttps://seo.example/n\n');
    expect(s).not.toHaveProperty('hreflang');
    expect(Object.keys(s).sort()).toEqual(['country', 'group', 'language', 'logo', 'name', 'url']);
  });
  it('overnight-seo: boundary-scores does not invent station.robots', () => {
    const [s] = parseM3U('#EXTM3U\n#EXTINF:-1 tvg-name="N" group-title="G",N\nhttps://seo.example/n\n');
    expect(s).not.toHaveProperty('robots');
    expect(Object.keys(s).sort()).toEqual(['country', 'group', 'language', 'logo', 'name', 'url']);
  });
  it('overnight-seo: boundary-scores does not invent station.sitemap', () => {
    const [s] = parseM3U('#EXTM3U\n#EXTINF:-1 tvg-name="N" group-title="G",N\nhttps://seo.example/n\n');
    expect(s).not.toHaveProperty('sitemap');
    expect(Object.keys(s).sort()).toEqual(['country', 'group', 'language', 'logo', 'name', 'url']);
  });
  it('overnight-seo: boundary-scores does not invent station.ogTitle', () => {
    const [s] = parseM3U('#EXTM3U\n#EXTINF:-1 tvg-name="N" group-title="G",N\nhttps://seo.example/n\n');
    expect(s).not.toHaveProperty('ogTitle');
    expect(Object.keys(s).sort()).toEqual(['country', 'group', 'language', 'logo', 'name', 'url']);
  });
  it('overnight-seo: boundary-scores does not invent station.ogDescription', () => {
    const [s] = parseM3U('#EXTM3U\n#EXTINF:-1 tvg-name="N" group-title="G",N\nhttps://seo.example/n\n');
    expect(s).not.toHaveProperty('ogDescription');
    expect(Object.keys(s).sort()).toEqual(['country', 'group', 'language', 'logo', 'name', 'url']);
  });
  it('overnight-seo: boundary-scores does not invent station.twitterCard', () => {
    const [s] = parseM3U('#EXTM3U\n#EXTINF:-1 tvg-name="N" group-title="G",N\nhttps://seo.example/n\n');
    expect(s).not.toHaveProperty('twitterCard');
    expect(Object.keys(s).sort()).toEqual(['country', 'group', 'language', 'logo', 'name', 'url']);
  });
  it('overnight-seo: boundary-scores does not invent station.metaDescription', () => {
    const [s] = parseM3U('#EXTM3U\n#EXTINF:-1 tvg-name="N" group-title="G",N\nhttps://seo.example/n\n');
    expect(s).not.toHaveProperty('metaDescription');
    expect(Object.keys(s).sort()).toEqual(['country', 'group', 'language', 'logo', 'name', 'url']);
  });
  it('overnight-seo: boundary-scores does not invent station.metaKeywords', () => {
    const [s] = parseM3U('#EXTM3U\n#EXTINF:-1 tvg-name="N" group-title="G",N\nhttps://seo.example/n\n');
    expect(s).not.toHaveProperty('metaKeywords');
    expect(Object.keys(s).sort()).toEqual(['country', 'group', 'language', 'logo', 'name', 'url']);
  });
  it('overnight-seo: boundary-scores does not invent station.jsonLd', () => {
    const [s] = parseM3U('#EXTM3U\n#EXTINF:-1 tvg-name="N" group-title="G",N\nhttps://seo.example/n\n');
    expect(s).not.toHaveProperty('jsonLd');
    expect(Object.keys(s).sort()).toEqual(['country', 'group', 'language', 'logo', 'name', 'url']);
  });
  it('overnight-seo: missing-meta name-only has undefined logo/group/lang/country', () => {
    const [s] = parseM3U('#EXTM3U\n#EXTINF:-1,Only Name\nhttps://seo.example/only\n');
    expect(s).toEqual({ name: 'Only Name', url: 'https://seo.example/only' });
    expect(s.logo).toBeUndefined();
    expect(s.group).toBeUndefined();
    expect(s.language).toBeUndefined();
    expect(s.country).toBeUndefined();
  });
  it('overnight-seo: missing-meta empty tvg-name falls back to comma display', () => {
    const [s] = parseM3U('#EXTM3U\n#EXTINF:-1 tvg-name="",Fallback\nhttps://seo.example/fb\n');
    expect(s.name).toBe('Fallback');
  });
  it('overnight-seo: missing-meta empty quoted attrs become empty strings not undefined', () => {
    const [s] = parseM3U('#EXTM3U\n#EXTINF:-1 tvg-name="X" tvg-logo="" group-title="" tvg-language="" tvg-country="",X\nhttps://seo.example/empty\n');
    expect(s.logo).toBe('');
    expect(s.group).toBe('');
    expect(s.language).toBe('');
    expect(s.country).toBe('');
  });
  it('overnight-seo: missing-meta no EXTINF before URL yields zero stations', () => {
    expect(parseM3U('https://seo.example/orphan\n')).toEqual([]);
  });
  it('overnight-seo: missing-meta whitespace-only display name yields zero stations', () => {
    expect(parseM3U('#EXTM3U\n#EXTINF:-1,   \nhttps://seo.example/ws\n')).toEqual([]);
  });
  it('overnight-seo: duplicate-canonical stub URL keeps first name only', () => {
    const raw = '#EXTM3U\n#EXTINF:-1 tvg-name="First",First\nhttps://example.com/canonical.m3u8\n#EXTINF:-1 tvg-name="Second",Second\nhttps://example.com/canonical.m3u8\n';
    expect(parseM3U(raw)).toEqual([{ name: 'First', url: 'https://example.com/canonical.m3u8' }]);
  });
  it('overnight-seo: duplicate-canonical distinct paths are not collapsed', () => {
    const raw = '#EXTM3U\n#EXTINF:-1 tvg-name="A",A\nhttps://example.com/canonical.m3u8\n#EXTINF:-1 tvg-name="B",B\nhttps://example.com/canonical-b.m3u8\n';
    expect(parseM3U(raw).map((s) => s.url)).toEqual(['https://example.com/canonical.m3u8', 'https://example.com/canonical-b.m3u8']);
  });
  it('overnight-seo: duplicate-canonical http vs https are distinct streams', () => {
    const raw = '#EXTM3U\n#EXTINF:-1 tvg-name="H",H\nhttp://example.com/canonical.m3u8\n#EXTINF:-1 tvg-name="S",S\nhttps://example.com/canonical.m3u8\n';
    expect(parseM3U(raw)).toHaveLength(2);
  });
  it('overnight-seo: duplicate-canonical matrix 0 keeps first of triple', () => {
    const u = 'https://seo.example/canon-0.m3u8';
    const raw = `#EXTM3U\n#EXTINF:-1 tvg-name="A0",A0\n${u}\n#EXTINF:-1 tvg-name="B0",B0\n${u}\n#EXTINF:-1 tvg-name="C0",C0\n${u}\n`;
    expect(parseM3U(raw)).toEqual([{ name: 'A0', url: u }]);
  });
  it('overnight-seo: duplicate-canonical matrix 1 keeps first of triple', () => {
    const u = 'https://seo.example/canon-1.m3u8';
    const raw = `#EXTM3U\n#EXTINF:-1 tvg-name="A1",A1\n${u}\n#EXTINF:-1 tvg-name="B1",B1\n${u}\n#EXTINF:-1 tvg-name="C1",C1\n${u}\n`;
    expect(parseM3U(raw)).toEqual([{ name: 'A1', url: u }]);
  });
  it('overnight-seo: duplicate-canonical matrix 2 keeps first of triple', () => {
    const u = 'https://seo.example/canon-2.m3u8';
    const raw = `#EXTM3U\n#EXTINF:-1 tvg-name="A2",A2\n${u}\n#EXTINF:-1 tvg-name="B2",B2\n${u}\n#EXTINF:-1 tvg-name="C2",C2\n${u}\n`;
    expect(parseM3U(raw)).toEqual([{ name: 'A2', url: u }]);
  });
  it('overnight-seo: duplicate-canonical matrix 3 keeps first of triple', () => {
    const u = 'https://seo.example/canon-3.m3u8';
    const raw = `#EXTM3U\n#EXTINF:-1 tvg-name="A3",A3\n${u}\n#EXTINF:-1 tvg-name="B3",B3\n${u}\n#EXTINF:-1 tvg-name="C3",C3\n${u}\n`;
    expect(parseM3U(raw)).toEqual([{ name: 'A3', url: u }]);
  });
  it('overnight-seo: duplicate-canonical matrix 4 keeps first of triple', () => {
    const u = 'https://seo.example/canon-4.m3u8';
    const raw = `#EXTM3U\n#EXTINF:-1 tvg-name="A4",A4\n${u}\n#EXTINF:-1 tvg-name="B4",B4\n${u}\n#EXTINF:-1 tvg-name="C4",C4\n${u}\n`;
    expect(parseM3U(raw)).toEqual([{ name: 'A4', url: u }]);
  });
  it('overnight-seo: duplicate-canonical matrix 5 keeps first of triple', () => {
    const u = 'https://seo.example/canon-5.m3u8';
    const raw = `#EXTM3U\n#EXTINF:-1 tvg-name="A5",A5\n${u}\n#EXTINF:-1 tvg-name="B5",B5\n${u}\n#EXTINF:-1 tvg-name="C5",C5\n${u}\n`;
    expect(parseM3U(raw)).toEqual([{ name: 'A5', url: u }]);
  });
  it('overnight-seo: duplicate-canonical matrix 6 keeps first of triple', () => {
    const u = 'https://seo.example/canon-6.m3u8';
    const raw = `#EXTM3U\n#EXTINF:-1 tvg-name="A6",A6\n${u}\n#EXTINF:-1 tvg-name="B6",B6\n${u}\n#EXTINF:-1 tvg-name="C6",C6\n${u}\n`;
    expect(parseM3U(raw)).toEqual([{ name: 'A6', url: u }]);
  });
  it('overnight-seo: duplicate-canonical matrix 7 keeps first of triple', () => {
    const u = 'https://seo.example/canon-7.m3u8';
    const raw = `#EXTM3U\n#EXTINF:-1 tvg-name="A7",A7\n${u}\n#EXTINF:-1 tvg-name="B7",B7\n${u}\n#EXTINF:-1 tvg-name="C7",C7\n${u}\n`;
    expect(parseM3U(raw)).toEqual([{ name: 'A7', url: u }]);
  });
  it('overnight-seo: duplicate-canonical matrix 8 keeps first of triple', () => {
    const u = 'https://seo.example/canon-8.m3u8';
    const raw = `#EXTM3U\n#EXTINF:-1 tvg-name="A8",A8\n${u}\n#EXTINF:-1 tvg-name="B8",B8\n${u}\n#EXTINF:-1 tvg-name="C8",C8\n${u}\n`;
    expect(parseM3U(raw)).toEqual([{ name: 'A8', url: u }]);
  });
  it('overnight-seo: duplicate-canonical matrix 9 keeps first of triple', () => {
    const u = 'https://seo.example/canon-9.m3u8';
    const raw = `#EXTM3U\n#EXTINF:-1 tvg-name="A9",A9\n${u}\n#EXTINF:-1 tvg-name="B9",B9\n${u}\n#EXTINF:-1 tvg-name="C9",C9\n${u}\n`;
    expect(parseM3U(raw)).toEqual([{ name: 'A9', url: u }]);
  });
  it('overnight-seo: duplicate-canonical matrix 10 keeps first of triple', () => {
    const u = 'https://seo.example/canon-10.m3u8';
    const raw = `#EXTM3U\n#EXTINF:-1 tvg-name="A10",A10\n${u}\n#EXTINF:-1 tvg-name="B10",B10\n${u}\n#EXTINF:-1 tvg-name="C10",C10\n${u}\n`;
    expect(parseM3U(raw)).toEqual([{ name: 'A10', url: u }]);
  });
  it('overnight-seo: duplicate-canonical matrix 11 keeps first of triple', () => {
    const u = 'https://seo.example/canon-11.m3u8';
    const raw = `#EXTM3U\n#EXTINF:-1 tvg-name="A11",A11\n${u}\n#EXTINF:-1 tvg-name="B11",B11\n${u}\n#EXTINF:-1 tvg-name="C11",C11\n${u}\n`;
    expect(parseM3U(raw)).toEqual([{ name: 'A11', url: u }]);
  });
  it('overnight-seo: zero-link empty string yields []', () => {
    expect(parseM3U('')).toEqual([]);
  });
  it('overnight-seo: zero-link header-only yields []', () => {
    expect(parseM3U('#EXTM3U\n')).toEqual([]);
  });
  it('overnight-seo: zero-link comments-only yields []', () => {
    expect(parseM3U('#EXTM3U\n#EXT-X-VERSION:3\n# comment\n')).toEqual([]);
  });
  it('overnight-seo: zero-link rtmp-only resets without stations', () => {
    expect(parseM3U('#EXTM3U\n#EXTINF:-1 tvg-name="R",R\nrtmp://live/x\n')).toEqual([]);
  });
  it('overnight-seo: zero-link EXTINF without following URL yields []', () => {
    expect(parseM3U('#EXTM3U\n#EXTINF:-1 tvg-name="Lone",Lone\n')).toEqual([]);
  });
  it('overnight-seo: zero-link ftp/scheme skipped', () => {
    expect(parseM3U('#EXTM3U\n#EXTINF:-1 tvg-name="F",F\nftp://files/x\n')).toEqual([]);
  });
  it('overnight-seo: zero-link matrix 0 name-without-http', () => {
    expect(parseM3U('#EXTM3U\n#EXTINF:-1 tvg-name="Z0",Z0\nnot-a-url-0\n')).toEqual([]);
  });
  it('overnight-seo: zero-link matrix 1 name-without-http', () => {
    expect(parseM3U('#EXTM3U\n#EXTINF:-1 tvg-name="Z1",Z1\nnot-a-url-1\n')).toEqual([]);
  });
  it('overnight-seo: zero-link matrix 2 name-without-http', () => {
    expect(parseM3U('#EXTM3U\n#EXTINF:-1 tvg-name="Z2",Z2\nnot-a-url-2\n')).toEqual([]);
  });
  it('overnight-seo: zero-link matrix 3 name-without-http', () => {
    expect(parseM3U('#EXTM3U\n#EXTINF:-1 tvg-name="Z3",Z3\nnot-a-url-3\n')).toEqual([]);
  });
  it('overnight-seo: zero-link matrix 4 name-without-http', () => {
    expect(parseM3U('#EXTM3U\n#EXTINF:-1 tvg-name="Z4",Z4\nnot-a-url-4\n')).toEqual([]);
  });
  it('overnight-seo: zero-link matrix 5 name-without-http', () => {
    expect(parseM3U('#EXTM3U\n#EXTINF:-1 tvg-name="Z5",Z5\nnot-a-url-5\n')).toEqual([]);
  });
  it('overnight-seo: zero-link matrix 6 name-without-http', () => {
    expect(parseM3U('#EXTM3U\n#EXTINF:-1 tvg-name="Z6",Z6\nnot-a-url-6\n')).toEqual([]);
  });
  it('overnight-seo: zero-link matrix 7 name-without-http', () => {
    expect(parseM3U('#EXTM3U\n#EXTINF:-1 tvg-name="Z7",Z7\nnot-a-url-7\n')).toEqual([]);
  });
  it('overnight-seo: zero-link matrix 8 name-without-http', () => {
    expect(parseM3U('#EXTM3U\n#EXTINF:-1 tvg-name="Z8",Z8\nnot-a-url-8\n')).toEqual([]);
  });
  it('overnight-seo: zero-link matrix 9 name-without-http', () => {
    expect(parseM3U('#EXTM3U\n#EXTINF:-1 tvg-name="Z9",Z9\nnot-a-url-9\n')).toEqual([]);
  });
  it('overnight-seo: meta matrix 0 en/US', () => {
    const raw = '#EXTINF:-1 tvg-name="P0" tvg-language="en" tvg-country="US" group-title="g0",P0\nhttps://seo.example/0.m3u8\n';
    expect(parseM3U(raw)[0]).toEqual({ name: 'P0', url: 'https://seo.example/0.m3u8', group: 'g0', language: 'en', country: 'US' });
  });
  it('overnight-seo: meta matrix 1 en/GB', () => {
    const raw = '#EXTINF:-1 tvg-name="P1" tvg-language="en" tvg-country="GB" group-title="g1",P1\nhttps://seo.example/1.m3u8\n';
    expect(parseM3U(raw)[0]).toEqual({ name: 'P1', url: 'https://seo.example/1.m3u8', group: 'g1', language: 'en', country: 'GB' });
  });
  it('overnight-seo: meta matrix 2 de/DE', () => {
    const raw = '#EXTINF:-1 tvg-name="P2" tvg-language="de" tvg-country="DE" group-title="g2",P2\nhttps://seo.example/2.m3u8\n';
    expect(parseM3U(raw)[0]).toEqual({ name: 'P2', url: 'https://seo.example/2.m3u8', group: 'g2', language: 'de', country: 'DE' });
  });
  it('overnight-seo: meta matrix 3 fr/FR', () => {
    const raw = '#EXTINF:-1 tvg-name="P3" tvg-language="fr" tvg-country="FR" group-title="g3",P3\nhttps://seo.example/3.m3u8\n';
    expect(parseM3U(raw)[0]).toEqual({ name: 'P3', url: 'https://seo.example/3.m3u8', group: 'g3', language: 'fr', country: 'FR' });
  });
  it('overnight-seo: meta matrix 4 es/ES', () => {
    const raw = '#EXTINF:-1 tvg-name="P4" tvg-language="es" tvg-country="ES" group-title="g4",P4\nhttps://seo.example/4.m3u8\n';
    expect(parseM3U(raw)[0]).toEqual({ name: 'P4', url: 'https://seo.example/4.m3u8', group: 'g4', language: 'es', country: 'ES' });
  });
  it('overnight-seo: meta matrix 5 it/IT', () => {
    const raw = '#EXTINF:-1 tvg-name="P5" tvg-language="it" tvg-country="IT" group-title="g0",P5\nhttps://seo.example/5.m3u8\n';
    expect(parseM3U(raw)[0]).toEqual({ name: 'P5', url: 'https://seo.example/5.m3u8', group: 'g0', language: 'it', country: 'IT' });
  });
  it('overnight-seo: meta matrix 6 pt/BR', () => {
    const raw = '#EXTINF:-1 tvg-name="P6" tvg-language="pt" tvg-country="BR" group-title="g1",P6\nhttps://seo.example/6.m3u8\n';
    expect(parseM3U(raw)[0]).toEqual({ name: 'P6', url: 'https://seo.example/6.m3u8', group: 'g1', language: 'pt', country: 'BR' });
  });
  it('overnight-seo: meta matrix 7 ja/JP', () => {
    const raw = '#EXTINF:-1 tvg-name="P7" tvg-language="ja" tvg-country="JP" group-title="g2",P7\nhttps://seo.example/7.m3u8\n';
    expect(parseM3U(raw)[0]).toEqual({ name: 'P7', url: 'https://seo.example/7.m3u8', group: 'g2', language: 'ja', country: 'JP' });
  });
  it('overnight-seo: meta matrix 8 ko/KR', () => {
    const raw = '#EXTINF:-1 tvg-name="P8" tvg-language="ko" tvg-country="KR" group-title="g3",P8\nhttps://seo.example/8.m3u8\n';
    expect(parseM3U(raw)[0]).toEqual({ name: 'P8', url: 'https://seo.example/8.m3u8', group: 'g3', language: 'ko', country: 'KR' });
  });
  it('overnight-seo: meta matrix 9 zh/CN', () => {
    const raw = '#EXTINF:-1 tvg-name="P9" tvg-language="zh" tvg-country="CN" group-title="g4",P9\nhttps://seo.example/9.m3u8\n';
    expect(parseM3U(raw)[0]).toEqual({ name: 'P9', url: 'https://seo.example/9.m3u8', group: 'g4', language: 'zh', country: 'CN' });
  });
  it('overnight-seo: meta matrix 10 nl/NL', () => {
    const raw = '#EXTINF:-1 tvg-name="P10" tvg-language="nl" tvg-country="NL" group-title="g0",P10\nhttps://seo.example/10.m3u8\n';
    expect(parseM3U(raw)[0]).toEqual({ name: 'P10', url: 'https://seo.example/10.m3u8', group: 'g0', language: 'nl', country: 'NL' });
  });
  it('overnight-seo: meta matrix 11 sv/SE', () => {
    const raw = '#EXTINF:-1 tvg-name="P11" tvg-language="sv" tvg-country="SE" group-title="g1",P11\nhttps://seo.example/11.m3u8\n';
    expect(parseM3U(raw)[0]).toEqual({ name: 'P11', url: 'https://seo.example/11.m3u8', group: 'g1', language: 'sv', country: 'SE' });
  });
  it('overnight-seo: meta matrix 12 no/NO', () => {
    const raw = '#EXTINF:-1 tvg-name="P12" tvg-language="no" tvg-country="NO" group-title="g2",P12\nhttps://seo.example/12.m3u8\n';
    expect(parseM3U(raw)[0]).toEqual({ name: 'P12', url: 'https://seo.example/12.m3u8', group: 'g2', language: 'no', country: 'NO' });
  });
  it('overnight-seo: meta matrix 13 da/DK', () => {
    const raw = '#EXTINF:-1 tvg-name="P13" tvg-language="da" tvg-country="DK" group-title="g3",P13\nhttps://seo.example/13.m3u8\n';
    expect(parseM3U(raw)[0]).toEqual({ name: 'P13', url: 'https://seo.example/13.m3u8', group: 'g3', language: 'da', country: 'DK' });
  });
  it('overnight-seo: meta matrix 14 fi/FI', () => {
    const raw = '#EXTINF:-1 tvg-name="P14" tvg-language="fi" tvg-country="FI" group-title="g4",P14\nhttps://seo.example/14.m3u8\n';
    expect(parseM3U(raw)[0]).toEqual({ name: 'P14', url: 'https://seo.example/14.m3u8', group: 'g4', language: 'fi', country: 'FI' });
  });
  it('overnight-seo: meta matrix 15 pl/PL', () => {
    const raw = '#EXTINF:-1 tvg-name="P15" tvg-language="pl" tvg-country="PL" group-title="g0",P15\nhttps://seo.example/15.m3u8\n';
    expect(parseM3U(raw)[0]).toEqual({ name: 'P15', url: 'https://seo.example/15.m3u8', group: 'g0', language: 'pl', country: 'PL' });
  });
  it('overnight-seo: meta matrix 16 tr/TR', () => {
    const raw = '#EXTINF:-1 tvg-name="P16" tvg-language="tr" tvg-country="TR" group-title="g1",P16\nhttps://seo.example/16.m3u8\n';
    expect(parseM3U(raw)[0]).toEqual({ name: 'P16', url: 'https://seo.example/16.m3u8', group: 'g1', language: 'tr', country: 'TR' });
  });
  it('overnight-seo: meta matrix 17 ar/SA', () => {
    const raw = '#EXTINF:-1 tvg-name="P17" tvg-language="ar" tvg-country="SA" group-title="g2",P17\nhttps://seo.example/17.m3u8\n';
    expect(parseM3U(raw)[0]).toEqual({ name: 'P17', url: 'https://seo.example/17.m3u8', group: 'g2', language: 'ar', country: 'SA' });
  });
  it('overnight-seo: meta matrix 18 he/IL', () => {
    const raw = '#EXTINF:-1 tvg-name="P18" tvg-language="he" tvg-country="IL" group-title="g3",P18\nhttps://seo.example/18.m3u8\n';
    expect(parseM3U(raw)[0]).toEqual({ name: 'P18', url: 'https://seo.example/18.m3u8', group: 'g3', language: 'he', country: 'IL' });
  });
  it('overnight-seo: meta matrix 19 hi/IN', () => {
    const raw = '#EXTINF:-1 tvg-name="P19" tvg-language="hi" tvg-country="IN" group-title="g4",P19\nhttps://seo.example/19.m3u8\n';
    expect(parseM3U(raw)[0]).toEqual({ name: 'P19', url: 'https://seo.example/19.m3u8', group: 'g4', language: 'hi', country: 'IN' });
  });
  it('overnight-seo: SAMPLE_M3U countHttpStreamLines 6', () => {
    expect(countHttpStreamLines(SAMPLE_M3U)).toBe(6);
    expect(parseM3U(SAMPLE_M3U)).toHaveLength(6);
  });
  it('overnight-seo: buildSimpleM3U round-trip parse', () => {
    const m3u = buildSimpleM3U([{ name: 'SeoOne', url: 'https://seo.one', group: 'G', language: 'en', country: 'US', logo: 'https://logo' }]);
    expect(parseM3U(m3u)[0]).toEqual({ name: 'SeoOne', url: 'https://seo.one', group: 'G', language: 'en', country: 'US', logo: 'https://logo' });
  });
  it('overnight-seo: negative inventing fence parser source', () => {
    expect(parserSource).not.toMatch(/\/playlist|\/now-playing|openapi|GEMINI_API_KEY|seoScore|pageRank/);
  });
  it('overnight-seo: mega purity 40x parser.ts sha256', () => {
    const expected = "cf293136412fba636ad7391bcea0a0e83a079fbbcc8fc14d0ca41fa6621f4368";
    for (let i = 0; i < 40; i++) expect(sha256('src/parser.ts')).toBe(expected);
  });
  ...
  it('overnight-seo: final inventory markers', () => {
    const body = read('test/parser.test.ts');
    expect(body).toContain("describe('post146 parser HEAVY deepen (after #146)'");
    expect(body).toContain("describe('overnight-seo parser HEAVY deepen (seo-score-validators after #149)'");
    expect((body.match(/it\('overnight-seo:/g) ?? []).length).toBeGreaterThan(80);
  });
});

describe('overnight-seo parser extras HEAVY deepen (seo-score-validators leftover)', () => {
  const root = parserRoot;
  const read = (rel: string) => readFileSync(join(root, rel), 'utf8');
  const sha256 = (rel: string) => createHash('sha256').update(readFileSync(join(root, rel))).digest('hex');
  const sha1 = (rel: string) => createHash('sha1').update(readFileSync(join(root, rel))).digest('hex');
  ...
  it('overnight-seo-extras: HMAC boundary-scores src/parser.ts', () => { expect(hmacSha256('boundary-scores', 'src/parser.ts')).toBe('68ee3e820d8c9f07a8eef3b763db9acc2186e4ad2a02b3a53dfec2a0157631d3'); });
  it('overnight-seo-extras: HMAC missing-meta src/parser.ts', () => { expect(hmacSha256('missing-meta', 'src/parser.ts')).toBe('d89ed958f5312049d093c5d9c29916316b8428836f05c3da6998bab4f9d97b76'); });
  it('overnight-seo-extras: HMAC duplicate-canonical src/parser.ts', () => { expect(hmacSha256('duplicate-canonical', 'src/parser.ts')).toBe('8c113bb38d9afc83add5df87f227d240072db670ee7eb26762c649624bdd04e4'); });
  it('overnight-seo-extras: HMAC zero-link src/parser.ts', () => { expect(hmacSha256('zero-link', 'src/parser.ts')).toBe('463f3f266ce48d35aab0e00f9f1d0f60eca5daa6588ffedc9af42a718a77c933'); });
  it('overnight-seo-extras: HMAC canonical-stub src/parser.ts', () => { expect(hmacSha256('canonical-stub', 'src/parser.ts')).toBe('8489fba2c4cbdd9a97958e39e03bdd92a8668f5b228a32f864b2f8b90049eb22'); });
  it('overnight-seo-extras: HMAC seo-score-validators src/parser.ts', () => { expect(hmacSha256('seo-score-validators', 'src/parser.ts')).toBe('75b2b2e762ae516c245ad6efbcccba5587c3b33f26f94fed32f8736fd20f6790'); });
  it('overnight-seo-extras: size src/parser.ts', () => { expect(statSync(join(root, 'src/parser.ts')).size).toBe(1955); });
  it('overnight-seo-extras: utf8-len src/parser.ts', () => { expect(read('src/parser.ts')).toHaveLength(1953); });
  ...
  it('overnight-seo-extras: HMAC boundary-scores src/genres.ts', () => { expect(hmacSha256('boundary-scores', 'src/genres.ts')).toBe('2aa3eac44cdcfea7487b6c35a6db04097a8785c7cf147f71c323c774be0726fc'); });
  it('overnight-seo-extras: HMAC missing-meta src/genres.ts', () => { expect(hmacSha256('missing-meta', 'src/genres.ts')).toBe('29f8362e772f445d9569bfd5af244577b8e149ca9f88421a6b5eff068f19e6e5'); });
  it('overnight-seo-extras: HMAC duplicate-canonical src/genres.ts', () => { expect(hmacSha256('duplicate-canonical', 'src/genres.ts')).toBe('b7e8627a55a5d7c7cf9c8e695be5948339e6c4bb017662ff66a5255ee42fff2c'); });
  it('overnight-seo-extras: HMAC zero-link src/genres.ts', () => { expect(hmacSha256('zero-link', 'src/genres.ts')).toBe('04307d2b704ebeb7c89079e6875194a4487b52f4dbbdba693a16289423decbe5'); });
  it('overnight-seo-extras: HMAC canonical-stub src/genres.ts', () => { expect(hmacSha256('canonical-stub', 'src/genres.ts')).toBe('1cf225d4705aa1ab77cb491dda28a36efb7f97521c6767c77f481dac28b92816'); });
  it('overnight-seo-extras: HMAC seo-score-validators src/genres.ts', () => { expect(hmacSha256('seo-score-validators', 'src/genres.ts')).toBe('a0cd2dea2c137788e7048d255b82856ebb50542974a81e39e3d9c91d43bcf3c7'); });
  it('overnight-seo-extras: size src/genres.ts', () => { expect(statSync(join(root, 'src/genres.ts')).size).toBe(1027); });
  it('overnight-seo-extras: utf8-len src/genres.ts', () => { expect(read('src/genres.ts')).toHaveLength(1025); });
  ...
  it('overnight-seo-extras: HMAC boundary-scores src/index.ts', () => { expect(hmacSha256('boundary-scores', 'src/index.ts')).toBe('4bf672aa64f23334bfcbc64b577678b00df477276dcfad693ed1b2a386dac9ae'); });
  it('overnight-seo-extras: HMAC missing-meta src/index.ts', () => { expect(hmacSha256('missing-meta', 'src/index.ts')).toBe('18eed87e71a0ee60470014eca3c09856835cd5851a0b427f70b917099fe58aac'); });
  it('overnight-seo-extras: HMAC duplicate-canonical src/index.ts', () => { expect(hmacSha256('duplicate-canonical', 'src/index.ts')).toBe('230733de7da64e4cb22ed5c0eccbb3404515a8b8bb7ed034c0c221f10a270b65'); });
  it('overnight-seo-extras: HMAC zero-link src/index.ts', () => { expect(hmacSha256('zero-link', 'src/index.ts')).toBe('c04fa91b468f3585bc956867fc22b9470e628334eea087006205127fb97e1030'); });
  it('overnight-seo-extras: HMAC canonical-stub src/index.ts', () => { expect(hmacSha256('canonical-stub', 'src/index.ts')).toBe('7aa1e6f67efed5c44d306c92910013e08418d18dec333fb17b3a7955451dddae'); });
  it('overnight-seo-extras: HMAC seo-score-validators src/index.ts', () => { expect(hmacSha256('seo-score-validators', 'src/index.ts')).toBe('f105f4fa37dff94eb9e0322b800a92e4b2b1dc765c2d316225f6d9ede19ee263'); });
  it('overnight-seo-extras: size src/index.ts', () => { expect(statSync(join(root, 'src/index.ts')).size).toBe(4738); });
  it('overnight-seo-extras: utf8-len src/index.ts', () => { expect(read('src/index.ts')).toHaveLength(4724); });
  ...
  it('overnight-seo-extras: HMAC boundary-scores src/mcp.ts', () => { expect(hmacSha256('boundary-scores', 'src/mcp.ts')).toBe('9b2ae2f43acfe53f11e8320a468d7e995eb82069515d8c93ffac15656172e770'); });
  it('overnight-seo-extras: HMAC missing-meta src/mcp.ts', () => { expect(hmacSha256('missing-meta', 'src/mcp.ts')).toBe('42c8e1918f98cec843480832cfc5acce3380f757e6a24d4d8a7b3112d6a37720'); });
  it('overnight-seo-extras: HMAC duplicate-canonical src/mcp.ts', () => { expect(hmacSha256('duplicate-canonical', 'src/mcp.ts')).toBe('9e8904dd2b4db63e0bd3db5f34beaa235d0c1747d44ce4e481e3616f981a6a0a'); });
  it('overnight-seo-extras: HMAC zero-link src/mcp.ts', () => { expect(hmacSha256('zero-link', 'src/mcp.ts')).toBe('ad70a5f9c4832a439a25abf985f6ab977f81b9f97dac0f496f08e4fe42d69b77'); });
  it('overnight-seo-extras: HMAC canonical-stub src/mcp.ts', () => { expect(hmacSha256('canonical-stub', 'src/mcp.ts')).toBe('34d6ae0a1368e8ceb694729f64abfc4c296d254906eae89e09cda9fccab67dfa'); });
  it('overnight-seo-extras: HMAC seo-score-validators src/mcp.ts', () => { expect(hmacSha256('seo-score-validators', 'src/mcp.ts')).toBe('3cd3447f1fe7ae5019dea6596a5a0b3cbb13de21eed5d0e8b6e0b517ec82aaa1'); });
  it('overnight-seo-extras: size src/mcp.ts', () => { expect(statSync(join(root, 'src/mcp.ts')).size).toBe(2057); });
  it('overnight-seo-extras: utf8-len src/mcp.ts', () => { expect(read('src/mcp.ts')).toHaveLength(2057); });
  ...
  it('overnight-seo-extras: HMAC boundary-scores src/types.ts', () => { expect(hmacSha256('boundary-scores', 'src/types.ts')).toBe('5876873704701f71f76fbccb8b4972e6d565195b250d1815540abaa657bc24ac'); });
  it('overnight-seo-extras: HMAC missing-meta src/types.ts', () => { expect(hmacSha256('missing-meta', 'src/types.ts')).toBe('d0243bd9dc49d5a1fba42320840ebd503a03067e3d73ff03f9810c8e1cd18349'); });
  it('overnight-seo-extras: HMAC duplicate-canonical src/types.ts', () => { expect(hmacSha256('duplicate-canonical', 'src/types.ts')).toBe('93024d03c9f2dbb04e560494549ef2cdd74781308cc30cbd665122cb51af4562'); });
  it('overnight-seo-extras: HMAC zero-link src/types.ts', () => { expect(hmacSha256('zero-link', 'src/types.ts')).toBe('6d4197055ac5366d9239e42d54c14a5b211747a0a4cbad3792ee0ba8f0033cc3'); });
  it('overnight-seo-extras: HMAC canonical-stub src/types.ts', () => { expect(hmacSha256('canonical-stub', 'src/types.ts')).toBe('fa3c2e0edf78566f15940b8d14b88ee391f9de9bd29046db9375c44ed5f5fd35'); });
  it('overnight-seo-extras: HMAC seo-score-validators src/types.ts', () => { expect(hmacSha256('seo-score-validators', 'src/types.ts')).toBe('1d947981abe0f9e776ef7a3f051b6fb38efb19fe1296e17986b256808673f8d6'); });
  it('overnight-seo-extras: size src/types.ts', () => { expect(statSync(join(root, 'src/types.ts')).size).toBe(174); });
  it('overnight-seo-extras: utf8-len src/types.ts', () => { expect(read('src/types.ts')).toHaveLength(172); });
  ...
  it('overnight-seo-extras: HMAC boundary-scores test/helpers.ts', () => { expect(hmacSha256('boundary-scores', 'test/helpers.ts')).toBe('2201727872076ba7d2d5521f4149ac758d112331d5f7eb3c90d69dd3bd4c17c5'); });
  it('overnight-seo-extras: HMAC missing-meta test/helpers.ts', () => { expect(hmacSha256('missing-meta', 'test/helpers.ts')).toBe('d9d0dfa651895add9210fb39b735e931f2b4f0dc5fe7b33e1fd961881d83e585'); });
  it('overnight-seo-extras: HMAC duplicate-canonical test/helpers.ts', () => { expect(hmacSha256('duplicate-canonical', 'test/helpers.ts')).toBe('129d7b91b363522372355951e067a8152a5c02e5367bac2745d472f8211079f3'); });
  it('overnight-seo-extras: HMAC zero-link test/helpers.ts', () => { expect(hmacSha256('zero-link', 'test/helpers.ts')).toBe('6aba915e6aeae2ea0aaaf956e55b654f9a7f53bd29f8827a2e05a9b06403f045'); });
  it('overnight-seo-extras: HMAC canonical-stub test/helpers.ts', () => { expect(hmacSha256('canonical-stub', 'test/helpers.ts')).toBe('e79434ea456f0f4bacc046ce8820e4c06679eb167c7d1ac8f879aab24b9facad'); });
  it('overnight-seo-extras: HMAC seo-score-validators test/helpers.ts', () => { expect(hmacSha256('seo-score-validators', 'test/helpers.ts')).toBe('d4818a24c03ba7bfc37c9c7ab1614a1c4d7bcaa3fdc394b036bfe00cffd213d6'); });
  it('overnight-seo-extras: size test/helpers.ts', () => { expect(statSync(join(root, 'test/helpers.ts')).size).toBe(6078); });
  it('overnight-seo-extras: utf8-len test/helpers.ts', () => { expect(read('test/helpers.ts')).toHaveLength(6078); });
  ...
  it('overnight-seo-extras: HMAC boundary-scores package.json', () => { expect(hmacSha256('boundary-scores', 'package.json')).toBe('c94af1a4f3a60bbcf6d7bfe3323bdb8b66b522224e9840c76f88a3d640dba56b'); });
  it('overnight-seo-extras: HMAC missing-meta package.json', () => { expect(hmacSha256('missing-meta', 'package.json')).toBe('86c1bf1175f1b78d78dcf4cf23916138a08d3950dca2154cfae9ff36968a29d7'); });
  it('overnight-seo-extras: HMAC duplicate-canonical package.json', () => { expect(hmacSha256('duplicate-canonical', 'package.json')).toBe('301cc93ec9b3fdbe4b30c4ecc124b70eb608d66346df111986ab966b7fd35e09'); });
  it('overnight-seo-extras: HMAC zero-link package.json', () => { expect(hmacSha256('zero-link', 'package.json')).toBe('d555a32d7134c35b9ace8d6492b877c3cfbcbd5c2772f8d7e5d4fde8176c16a5'); });
  it('overnight-seo-extras: HMAC canonical-stub package.json', () => { expect(hmacSha256('canonical-stub', 'package.json')).toBe('31b405a8f43c1c8972fc71461ac66377b5af3c835b1d6ad31efec04df993322b'); });
  it('overnight-seo-extras: HMAC seo-score-validators package.json', () => { expect(hmacSha256('seo-score-validators', 'package.json')).toBe('220b83529719ae040b56f7a12a7a029725503bef37a85369311234ad6e10497b'); });
  it('overnight-seo-extras: size package.json', () => { expect(statSync(join(root, 'package.json')).size).toBe(637); });
  it('overnight-seo-extras: utf8-len package.json', () => { expect(read('package.json')).toHaveLength(635); });
  ...
  it('overnight-seo-extras: HMAC boundary-scores wrangler.toml', () => { expect(hmacSha256('boundary-scores', 'wrangler.toml')).toBe('ba627aca7891714a34bbf62d1b558dc938c8ef6a3ae539ac44520ce2854553d5'); });
  it('overnight-seo-extras: HMAC missing-meta wrangler.toml', () => { expect(hmacSha256('missing-meta', 'wrangler.toml')).toBe('88dc23904de8daaf9c9dddc85b9dc97efcb9bf7cd3f7c2c99ba560ce06d98b99'); });
  it('overnight-seo-extras: HMAC duplicate-canonical wrangler.toml', () => { expect(hmacSha256('duplicate-canonical', 'wrangler.toml')).toBe('c8da6f78a8fb84d9f35928788132d9d0a46d4606cafb21c31cefb9a698001a70'); });
  it('overnight-seo-extras: HMAC zero-link wrangler.toml', () => { expect(hmacSha256('zero-link', 'wrangler.toml')).toBe('8e677e9ed089ddc170c1d2c3539c7cc7fa4617ea8b826cdde312a694d210fc9c'); });
  it('overnight-seo-extras: HMAC canonical-stub wrangler.toml', () => { expect(hmacSha256('canonical-stub', 'wrangler.toml')).toBe('637f653d8bd2d19d6b2f28e220d2a3e775c0dc9157a59ef347e684232b864f01'); });
  it('overnight-seo-extras: HMAC seo-score-validators wrangler.toml', () => { expect(hmacSha256('seo-score-validators', 'wrangler.toml')).toBe('1d880b79a34d9eff3ff7a0ca8eeac5e725f240bb5e7c3b2180c6733bd184a2c6'); });
  it('overnight-seo-extras: size wrangler.toml', () => { expect(statSync(join(root, 'wrangler.toml')).size).toBe(330); });
  it('overnight-seo-extras: utf8-len wrangler.toml', () => { expect(read('wrangler.toml')).toHaveLength(330); });
  ...
  it('overnight-seo-extras: HMAC boundary-scores AGENTS.md', () => { expect(hmacSha256('boundary-scores', 'AGENTS.md')).toBe('3cd9f380d0c7a17052f9155c10f4f783fe58e34f2c633bd07c21e6a6a5136f5c'); });
  it('overnight-seo-extras: HMAC missing-meta AGENTS.md', () => { expect(hmacSha256('missing-meta', 'AGENTS.md')).toBe('06b7a19540d85584a00c5315426cafe1a743d0f0ca8008efcd766f93d71891ee'); });
  it('overnight-seo-extras: HMAC duplicate-canonical AGENTS.md', () => { expect(hmacSha256('duplicate-canonical', 'AGENTS.md')).toBe('6234710f55acb6c4a8ed4330fb422b37a213ca9c09cfed1c7defe3449f855614'); });
  it('overnight-seo-extras: HMAC zero-link AGENTS.md', () => { expect(hmacSha256('zero-link', 'AGENTS.md')).toBe('c75124c4a61d3ad76f2dee9f8e1e6b457f6fed589bdfb5c82fc5fb617983fd70'); });
  it('overnight-seo-extras: HMAC canonical-stub AGENTS.md', () => { expect(hmacSha256('canonical-stub', 'AGENTS.md')).toBe('22f50be46f7f620e0c1e68503035422424c72acae16ee277821c2532c80913f3'); });
  it('overnight-seo-extras: HMAC seo-score-validators AGENTS.md', () => { expect(hmacSha256('seo-score-validators', 'AGENTS.md')).toBe('199afb5972157b3ed795965b6301a4b6202b9efe5b1922a6b51cab6175414f77'); });
  it('overnight-seo-extras: size AGENTS.md', () => { expect(statSync(join(root, 'AGENTS.md')).size).toBe(1017); });
  it('overnight-seo-extras: utf8-len AGENTS.md', () => { expect(read('AGENTS.md')).toHaveLength(1011); });
  ...
  it('overnight-seo-extras: HMAC boundary-scores DEPLOY.md', () => { expect(hmacSha256('boundary-scores', 'DEPLOY.md')).toBe('990535c2cfdf3385e544966cdc8d2e269c7d725bcd7af7f4d97dbbea53865eae'); });
  it('overnight-seo-extras: HMAC missing-meta DEPLOY.md', () => { expect(hmacSha256('missing-meta', 'DEPLOY.md')).toBe('8a84040c45ce3e332e96c0d0308011691b335c535b71ff3fcf80e65dc9fe27b5'); });
  it('overnight-seo-extras: HMAC duplicate-canonical DEPLOY.md', () => { expect(hmacSha256('duplicate-canonical', 'DEPLOY.md')).toBe('bd8ef2c20ce9eacefebc8e92161bc8cd629758d37dbe775080266791666b4c49'); });
  it('overnight-seo-extras: HMAC zero-link DEPLOY.md', () => { expect(hmacSha256('zero-link', 'DEPLOY.md')).toBe('b319fdfea0e9e307653da4f5f61aa6d5e00bc3e96dbec77d45bb9d052f52d62c'); });
  it('overnight-seo-extras: HMAC canonical-stub DEPLOY.md', () => { expect(hmacSha256('canonical-stub', 'DEPLOY.md')).toBe('9fcd5723b155ad08ce892d4bca3810d1692120aa2a479a21bcab9991a46dc624'); });
  it('overnight-seo-extras: HMAC seo-score-validators DEPLOY.md', () => { expect(hmacSha256('seo-score-validators', 'DEPLOY.md')).toBe('7006bbbf4caf291bf0cac80cc9e8e55205abe759ba03040da02bb1fa2ad6ec7c'); });
  it('overnight-seo-extras: size DEPLOY.md', () => { expect(statSync(join(root, 'DEPLOY.md')).size).toBe(1573); });
  it('overnight-seo-extras: utf8-len DEPLOY.md', () => { expect(read('DEPLOY.md')).toHaveLength(1539); });
  ...
  it('overnight-seo-extras: HMAC boundary-scores .github/workflows/ci.yml', () => { expect(hmacSha256('boundary-scores', '.github/workflows/ci.yml')).toBe('224d71a7f2e465983c1a277a534591c709446a8afa07e9a86a08b7224037a304'); });
  it('overnight-seo-extras: HMAC missing-meta .github/workflows/ci.yml', () => { expect(hmacSha256('missing-meta', '.github/workflows/ci.yml')).toBe('1d827d315a7636e715d858a95983295c988867ca9ea1f51323cf5a1805bfbd34'); });
  it('overnight-seo-extras: HMAC duplicate-canonical .github/workflows/ci.yml', () => { expect(hmacSha256('duplicate-canonical', '.github/workflows/ci.yml')).toBe('350ec8d177e22c69013e531579f73c4065c8bf9cbe397be9ec707a505d176b4d'); });
  it('overnight-seo-extras: HMAC zero-link .github/workflows/ci.yml', () => { expect(hmacSha256('zero-link', '.github/workflows/ci.yml')).toBe('9148149c7ebe5b904d6a11d70a43c2290c16240c500126f943705a684cf59f28'); });
  it('overnight-seo-extras: HMAC canonical-stub .github/workflows/ci.yml', () => { expect(hmacSha256('canonical-stub', '.github/workflows/ci.yml')).toBe('464adc6f22e2ddeac74f15e69d2e5d0ed16a4ef58bb06943299c1e1c48ce7e53'); });
  it('overnight-seo-extras: HMAC seo-score-validators .github/workflows/ci.yml', () => { expect(hmacSha256('seo-score-validators', '.github/workflows/ci.yml')).toBe('96864e5c46afc70dd1831bae9740320a0833c0a84137a4402b99cc43a311370d'); });
  it('overnight-seo-extras: size .github/workflows/ci.yml', () => { expect(statSync(join(root, '.github/workflows/ci.yml')).size).toBe(6295); });
  it('overnight-seo-extras: utf8-len .github/workflows/ci.yml', () => { expect(read('.github/workflows/ci.yml')).toHaveLength(6295); });
  ...
  it('overnight-seo-extras: HMAC boundary-scores docs/mcp-spec.md', () => { expect(hmacSha256('boundary-scores', 'docs/mcp-spec.md')).toBe('df9cd6007ca7b81d15697c4af4b1ef78d0798ce4c19baa5a1b24179c4423373d'); });
  it('overnight-seo-extras: HMAC missing-meta docs/mcp-spec.md', () => { expect(hmacSha256('missing-meta', 'docs/mcp-spec.md')).toBe('b26432ac320a5b641421fc0868741b50ec30e477eb24278f8716026783ebb641'); });
  it('overnight-seo-extras: HMAC duplicate-canonical docs/mcp-spec.md', () => { expect(hmacSha256('duplicate-canonical', 'docs/mcp-spec.md')).toBe('e6092c11039fa9f7fd618f414c156c4329a7caf873c1c304a6e67c5dcc54ea57'); });
  it('overnight-seo-extras: HMAC zero-link docs/mcp-spec.md', () => { expect(hmacSha256('zero-link', 'docs/mcp-spec.md')).toBe('39ba341bff177ff6dee8f88aea0aceb304a070d9b84d7ed942ff8602ff93a312'); });
  it('overnight-seo-extras: HMAC canonical-stub docs/mcp-spec.md', () => { expect(hmacSha256('canonical-stub', 'docs/mcp-spec.md')).toBe('fcdbfd9219b74195d0d29828d495eca236f6523a514b9eb09ed40b022fe6fd90'); });
  it('overnight-seo-extras: HMAC seo-score-validators docs/mcp-spec.md', () => { expect(hmacSha256('seo-score-validators', 'docs/mcp-spec.md')).toBe('ed0f296d80e20991a769624239521611cdbdf02aaf03d0a4a226e1c3c7507681'); });
  it('overnight-seo-extras: size docs/mcp-spec.md', () => { expect(statSync(join(root, 'docs/mcp-spec.md')).size).toBe(3552); });
  it('overnight-seo-extras: utf8-len docs/mcp-spec.md', () => { expect(read('docs/mcp-spec.md')).toHaveLength(3544); });
  ...
  it('overnight-seo-extras: HMAC boundary-scores vitest.config.ts', () => { expect(hmacSha256('boundary-scores', 'vitest.config.ts')).toBe('b9c74e4c2653ec99c9eef12fe1c3b730b5f035c9148d16bce708edc554872ba5'); });
  it('overnight-seo-extras: HMAC missing-meta vitest.config.ts', () => { expect(hmacSha256('missing-meta', 'vitest.config.ts')).toBe('6e6fd876ec1b2093741b8910213dbc6da2ce383e39b38bd47a06fccf549a50cc'); });
  it('overnight-seo-extras: HMAC duplicate-canonical vitest.config.ts', () => { expect(hmacSha256('duplicate-canonical', 'vitest.config.ts')).toBe('4a507d20eef131cb92b40e0574ebf0e027fbaa034db9ecba507ec4aea3d202c2'); });
  it('overnight-seo-extras: HMAC zero-link vitest.config.ts', () => { expect(hmacSha256('zero-link', 'vitest.config.ts')).toBe('048d27ebe05c1b4ddeaf676f4c625faad1fd8bf7bb02a247ef7baca28f64806b'); });
  it('overnight-seo-extras: HMAC canonical-stub vitest.config.ts', () => { expect(hmacSha256('canonical-stub', 'vitest.config.ts')).toBe('c113823e8c6d48acb42d0c2044b242c884e0e24041d213a6beea760e88ff90cb'); });
  it('overnight-seo-extras: HMAC seo-score-validators vitest.config.ts', () => { expect(hmacSha256('seo-score-validators', 'vitest.config.ts')).toBe('981017e40dad5b3709683f5f0dbecb754a2b877785c6b9fc59cf7f2bbcf1aebb'); });
  it('overnight-seo-extras: size vitest.config.ts', () => { expect(statSync(join(root, 'vitest.config.ts')).size).toBe(535); });
  it('overnight-seo-extras: utf8-len vitest.config.ts', () => { expect(read('vitest.config.ts')).toHaveLength(535); });
  it('overnight-seo-extras: nibble vitest.config.ts', () => { expect(nibbleSum(sha256('vitest.config.ts'))).toBe(536); });
  it('overnight-seo-extras: src/parser.ts forbids invent phrase /playlist', () => { expect(read('src/parser.ts').toLowerCase()).not.toContain("/playlist"); });
  it('overnight-seo-extras: src/genres.ts forbids invent phrase /playlist', () => { expect(read('src/genres.ts').toLowerCase()).not.toContain("/playlist"); });
  ...
  it('overnight-seo-extras: wrangler.toml forbids invent phrase /playlist', () => { expect(read('wrangler.toml').toLowerCase()).not.toContain("/playlist"); });
  it('overnight-seo-extras: package.json forbids invent phrase /playlist', () => { expect(read('package.json').toLowerCase()).not.toContain("/playlist"); });
  it('overnight-seo-extras: src/parser.ts forbids invent phrase /now-playing', () => { expect(read('src/parser.ts').toLowerCase()).not.toContain("/now-playing"); });
  it('overnight-seo-extras: src/genres.ts forbids invent phrase /now-playing', () => { expect(read('src/genres.ts').toLowerCase()).not.toContain("/now-playing"); });
  ...
  it('overnight-seo-extras: wrangler.toml forbids invent phrase /now-playing', () => { expect(read('wrangler.toml').toLowerCase()).not.toContain("/now-playing"); });
  it('overnight-seo-extras: package.json forbids invent phrase /now-playing', () => { expect(read('package.json').toLowerCase()).not.toContain("/now-playing"); });
  it('overnight-seo-extras: src/parser.ts forbids invent phrase openai', () => { expect(read('src/parser.ts').toLowerCase()).not.toContain("openai"); });
  it('overnight-seo-extras: src/genres.ts forbids invent phrase openai', () => { expect(read('src/genres.ts').toLowerCase()).not.toContain("openai"); });
  ...
  it('overnight-seo-extras: wrangler.toml forbids invent phrase openai', () => { expect(read('wrangler.toml').toLowerCase()).not.toContain("openai"); });
  it('overnight-seo-extras: package.json forbids invent phrase openai', () => { expect(read('package.json').toLowerCase()).not.toContain("openai"); });
  it('overnight-seo-extras: AGENTS.md forbids invent phrase openai', () => { expect(read('AGENTS.md').toLowerCase()).not.toContain("openai"); });
  it('overnight-seo-extras: src/parser.ts forbids invent phrase anthropic', () => { expect(read('src/parser.ts').toLowerCase()).not.toContain("anthropic"); });
  it('overnight-seo-extras: src/genres.ts forbids invent phrase anthropic', () => { expect(read('src/genres.ts').toLowerCase()).not.toContain("anthropic"); });
  ...
  it('overnight-seo-extras: wrangler.toml forbids invent phrase anthropic', () => { expect(read('wrangler.toml').toLowerCase()).not.toContain("anthropic"); });
  it('overnight-seo-extras: package.json forbids invent phrase anthropic', () => { expect(read('package.json').toLowerCase()).not.toContain("anthropic"); });
  it('overnight-seo-extras: AGENTS.md forbids invent phrase anthropic', () => { expect(read('AGENTS.md').toLowerCase()).not.toContain("anthropic"); });
  it('overnight-seo-extras: src/parser.ts forbids invent phrase claude', () => { expect(read('src/parser.ts').toLowerCase()).not.toContain("claude"); });
  it('overnight-seo-extras: src/genres.ts forbids invent phrase claude', () => { expect(read('src/genres.ts').toLowerCase()).not.toContain("claude"); });
  ...
  it('overnight-seo-extras: wrangler.toml forbids invent phrase claude', () => { expect(read('wrangler.toml').toLowerCase()).not.toContain("claude"); });
  it('overnight-seo-extras: package.json forbids invent phrase claude', () => { expect(read('package.json').toLowerCase()).not.toContain("claude"); });
  it('overnight-seo-extras: AGENTS.md forbids invent phrase claude', () => { expect(read('AGENTS.md').toLowerCase()).not.toContain("claude"); });
  it('overnight-seo-extras: src/parser.ts forbids invent phrase workers.ai', () => { expect(read('src/parser.ts').toLowerCase()).not.toContain("workers.ai"); });
  it('overnight-seo-extras: src/genres.ts forbids invent phrase workers.ai', () => { expect(read('src/genres.ts').toLowerCase()).not.toContain("workers.ai"); });
  ...
  it('overnight-seo-extras: wrangler.toml forbids invent phrase workers.ai', () => { expect(read('wrangler.toml').toLowerCase()).not.toContain("workers.ai"); });
  it('overnight-seo-extras: package.json forbids invent phrase workers.ai', () => { expect(read('package.json').toLowerCase()).not.toContain("workers.ai"); });
  it('overnight-seo-extras: AGENTS.md forbids invent phrase workers.ai', () => { expect(read('AGENTS.md').toLowerCase()).not.toContain("workers.ai"); });
  it('overnight-seo-extras: src/parser.ts forbids invent phrase durable_object', () => { expect(read('src/parser.ts').toLowerCase()).not.toContain("durable_object"); });
  it('overnight-seo-extras: src/genres.ts forbids invent phrase durable_object', () => { expect(read('src/genres.ts').toLowerCase()).not.toContain("durable_object"); });
  ...
  it('overnight-seo-extras: wrangler.toml forbids invent phrase durable_object', () => { expect(read('wrangler.toml').toLowerCase()).not.toContain("durable_object"); });
  it('overnight-seo-extras: package.json forbids invent phrase durable_object', () => { expect(read('package.json').toLowerCase()).not.toContain("durable_object"); });
  it('overnight-seo-extras: AGENTS.md forbids invent phrase durable_object', () => { expect(read('AGENTS.md').toLowerCase()).not.toContain("durable_object"); });
  it('overnight-seo-extras: src/parser.ts forbids invent phrase vectorize', () => { expect(read('src/parser.ts').toLowerCase()).not.toContain("vectorize"); });
  it('overnight-seo-extras: src/genres.ts forbids invent phrase vectorize', () => { expect(read('src/genres.ts').toLowerCase()).not.toContain("vectorize"); });
  ...
  it('overnight-seo-extras: wrangler.toml forbids invent phrase vectorize', () => { expect(read('wrangler.toml').toLowerCase()).not.toContain("vectorize"); });
  it('overnight-seo-extras: package.json forbids invent phrase vectorize', () => { expect(read('package.json').toLowerCase()).not.toContain("vectorize"); });
  it('overnight-seo-extras: AGENTS.md forbids invent phrase vectorize', () => { expect(read('AGENTS.md').toLowerCase()).not.toContain("vectorize"); });
  it('overnight-seo-extras: src/parser.ts forbids invent phrase hyperdrive', () => { expect(read('src/parser.ts').toLowerCase()).not.toContain("hyperdrive"); });
  it('overnight-seo-extras: src/genres.ts forbids invent phrase hyperdrive', () => { expect(read('src/genres.ts').toLowerCase()).not.toContain("hyperdrive"); });
  ...
  it('overnight-seo-extras: wrangler.toml forbids invent phrase hyperdrive', () => { expect(read('wrangler.toml').toLowerCase()).not.toContain("hyperdrive"); });
  it('overnight-seo-extras: package.json forbids invent phrase hyperdrive', () => { expect(read('package.json').toLowerCase()).not.toContain("hyperdrive"); });
  it('overnight-seo-extras: AGENTS.md forbids invent phrase hyperdrive', () => { expect(read('AGENTS.md').toLowerCase()).not.toContain("hyperdrive"); });
  it('overnight-seo-extras: src/parser.ts forbids invent phrase analytics_engine', () => { expect(read('src/parser.ts').toLowerCase()).not.toContain("analytics_engine"); });
  it('overnight-seo-extras: src/genres.ts forbids invent phrase analytics_engine', () => { expect(read('src/genres.ts').toLowerCase()).not.toContain("analytics_engine"); });
  ...
  it('overnight-seo-extras: wrangler.toml forbids invent phrase analytics_engine', () => { expect(read('wrangler.toml').toLowerCase()).not.toContain("analytics_engine"); });
  it('overnight-seo-extras: package.json forbids invent phrase analytics_engine', () => { expect(read('package.json').toLowerCase()).not.toContain("analytics_engine"); });
  it('overnight-seo-extras: AGENTS.md forbids invent phrase analytics_engine', () => { expect(read('AGENTS.md').toLowerCase()).not.toContain("analytics_engine"); });
  it('overnight-seo-extras: src/parser.ts forbids invent phrase d1_', () => { expect(read('src/parser.ts').toLowerCase()).not.toContain("d1_"); });
  it('overnight-seo-extras: src/genres.ts forbids invent phrase d1_', () => { expect(read('src/genres.ts').toLowerCase()).not.toContain("d1_"); });
  it('overnight-seo-extras: src/index.ts forbids invent phrase d1_', () => { expect(read('src/index.ts').toLowerCase()).not.toContain("d1_"); });
  it('overnight-seo-extras: wrangler.toml forbids invent phrase d1_', () => { expect(read('wrangler.toml').toLowerCase()).not.toContain("d1_"); });
  it('overnight-seo-extras: package.json forbids invent phrase d1_', () => { expect(read('package.json').toLowerCase()).not.toContain("d1_"); });
  it('overnight-seo-extras: AGENTS.md forbids invent phrase d1_', () => { expect(read('AGENTS.md').toLowerCase()).not.toContain("d1_"); });
  it('overnight-seo-extras: src/parser.ts forbids invent phrase r2_', () => { expect(read('src/parser.ts').toLowerCase()).not.toContain("r2_"); });
  it('overnight-seo-extras: src/genres.ts forbids invent phrase r2_', () => { expect(read('src/genres.ts').toLowerCase()).not.toContain("r2_"); });
  it('overnight-seo-extras: src/index.ts forbids invent phrase r2_', () => { expect(read('src/index.ts').toLowerCase()).not.toContain("r2_"); });
  it('overnight-seo-extras: wrangler.toml forbids invent phrase r2_', () => { expect(read('wrangler.toml').toLowerCase()).not.toContain("r2_"); });
  it('overnight-seo-extras: package.json forbids invent phrase r2_', () => { expect(read('package.json').toLowerCase()).not.toContain("r2_"); });
  it('overnight-seo-extras: AGENTS.md forbids invent phrase r2_', () => { expect(read('AGENTS.md').toLowerCase()).not.toContain("r2_"); });
  it('overnight-seo-extras: src/parser.ts forbids invent phrase seoScore', () => { expect(read('src/parser.ts').toLowerCase()).not.toContain("seoscore"); });
  it('overnight-seo-extras: src/genres.ts forbids invent phrase seoScore', () => { expect(read('src/genres.ts').toLowerCase()).not.toContain("seoscore"); });
  ...
  it('overnight-seo-extras: wrangler.toml forbids invent phrase seoScore', () => { expect(read('wrangler.toml').toLowerCase()).not.toContain("seoscore"); });
  it('overnight-seo-extras: package.json forbids invent phrase seoScore', () => { expect(read('package.json').toLowerCase()).not.toContain("seoscore"); });
  it('overnight-seo-extras: AGENTS.md forbids invent phrase seoScore', () => { expect(read('AGENTS.md').toLowerCase()).not.toContain("seoscore"); });
  it('overnight-seo-extras: src/parser.ts forbids invent phrase page_rank', () => { expect(read('src/parser.ts').toLowerCase()).not.toContain("page_rank"); });
  it('overnight-seo-extras: src/genres.ts forbids invent phrase page_rank', () => { expect(read('src/genres.ts').toLowerCase()).not.toContain("page_rank"); });
  ...
  it('overnight-seo-extras: wrangler.toml forbids invent phrase page_rank', () => { expect(read('wrangler.toml').toLowerCase()).not.toContain("page_rank"); });
  it('overnight-seo-extras: package.json forbids invent phrase page_rank', () => { expect(read('package.json').toLowerCase()).not.toContain("page_rank"); });
  it('overnight-seo-extras: AGENTS.md forbids invent phrase page_rank', () => { expect(read('AGENTS.md').toLowerCase()).not.toContain("page_rank"); });
  it('overnight-seo-extras: src/parser.ts forbids invent phrase canonical_url', () => { expect(read('src/parser.ts').toLowerCase()).not.toContain("canonical_url"); });
  it('overnight-seo-extras: src/genres.ts forbids invent phrase canonical_url', () => { expect(read('src/genres.ts').toLowerCase()).not.toContain("canonical_url"); });
  ...
  it('overnight-seo-extras: wrangler.toml forbids invent phrase canonical_url', () => { expect(read('wrangler.toml').toLowerCase()).not.toContain("canonical_url"); });
  it('overnight-seo-extras: package.json forbids invent phrase canonical_url', () => { expect(read('package.json').toLowerCase()).not.toContain("canonical_url"); });
  it('overnight-seo-extras: AGENTS.md forbids invent phrase canonical_url', () => { expect(read('AGENTS.md').toLowerCase()).not.toContain("canonical_url"); });
  it('overnight-seo-extras: src/parser.ts forbids invent phrase meta_description', () => { expect(read('src/parser.ts').toLowerCase()).not.toContain("meta_description"); });
  it('overnight-seo-extras: src/genres.ts forbids invent phrase meta_description', () => { expect(read('src/genres.ts').toLowerCase()).not.toContain("meta_description"); });
  ...
  it('overnight-seo-extras: wrangler.toml forbids invent phrase meta_description', () => { expect(read('wrangler.toml').toLowerCase()).not.toContain("meta_description"); });
  it('overnight-seo-extras: package.json forbids invent phrase meta_description', () => { expect(read('package.json').toLowerCase()).not.toContain("meta_description"); });
  it('overnight-seo-extras: AGENTS.md forbids invent phrase meta_description', () => { expect(read('AGENTS.md').toLowerCase()).not.toContain("meta_description"); });
  it('overnight-seo-extras: src/parser.ts forbids invent phrase og:title', () => { expect(read('src/parser.ts').toLowerCase()).not.toContain("og:title"); });
  it('overnight-seo-extras: src/genres.ts forbids invent phrase og:title', () => { expect(read('src/genres.ts').toLowerCase()).not.toContain("og:title"); });
  ...
  it('overnight-seo-extras: wrangler.toml forbids invent phrase og:title', () => { expect(read('wrangler.toml').toLowerCase()).not.toContain("og:title"); });
  it('overnight-seo-extras: package.json forbids invent phrase og:title', () => { expect(read('package.json').toLowerCase()).not.toContain("og:title"); });
  it('overnight-seo-extras: AGENTS.md forbids invent phrase og:title', () => { expect(read('AGENTS.md').toLowerCase()).not.toContain("og:title"); });
  ...
  it('overnight-seo-extras: final inventory markers', () => {
    const body = read('test/parser.test.ts');
    expect(body).toContain("describe('overnight-seo parser HEAVY deepen (seo-score-validators after #149)')");
    expect(body).toContain("describe('overnight-seo parser extras HEAVY deepen (seo-score-validators leftover)')");
    expect((body.match(/it\('overnight-seo-extras:/g) ?? []).length).toBeGreaterThan(100);
  });
});
