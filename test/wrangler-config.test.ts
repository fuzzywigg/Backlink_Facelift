...
describe('post134 wrangler HEAVY deepen (after #134)', () => {
  ...
  it('post134: locks src/types.ts char-class tabs/nl/comma/colon/quotes', () => {
  ...
  it('post134: locks src/types.ts HMAC-SHA1/MD5 key post134', () => {
    expect(createHmac('sha1', 'post134').update(readFileSync(join(root, 'src/types.ts'))).digest('hex')).toBe('db2994d333a1fc53ce70cd131c9c860f103ada1c');
    expect(createHmac('md5', 'post134').update(readFileSync(join(root, 'src/types.ts'))).digest('hex')).toBe('67b7f4d573e8e4c48f6540c06a5b2916');
  });
  ...
  it('post134: locks src/index.ts char-class tabs/nl/comma/colon/quotes', () => {
  ...
  it('post134: locks src/index.ts HMAC-SHA1/MD5 key post134', () => {
    expect(createHmac('sha1', 'post134').update(readFileSync(join(root, 'src/index.ts'))).digest('hex')).toBe('69adc3c9bfd30bb8a8b43634491ea610df14eecf');
    expect(createHmac('md5', 'post134').update(readFileSync(join(root, 'src/index.ts'))).digest('hex')).toBe('1ef6099b2d01f3630e9a62dabcd8021e');
  });
  ...
  it('post134: locks DEPLOY.md char-class tabs/nl/comma/colon/quotes', () => {
  ...
  it('post134: locks DEPLOY.md HMAC-SHA1/MD5 key post134', () => {
    expect(createHmac('sha1', 'post134').update(readFileSync(join(root, 'DEPLOY.md'))).digest('hex')).toBe('ffc442a68a7ecf1de22c3c816506b96a2fdaa002');
    expect(createHmac('md5', 'post134').update(readFileSync(join(root, 'DEPLOY.md'))).digest('hex')).toBe('171db8161807910cb932f772b8663856');
  });
  ...
  it('post134: wrangler name/main/compat pins', () => {
    const w = read('wrangler.toml');
    expect(w).toMatch(/^\s*name\s*=\s*"backlink"/m);
    expect(w).toMatch(/^\s*main\s*=\s*"src\/index\.ts"/m);
    expect(w).toMatch(/^\s*compatibility_date\s*=\s*"2025-01-01"/m);
  });

  it('post134: KV CATALOG_CACHE id pin without secret invent', () => {
    const w = read('wrangler.toml');
    expect(w).toContain('binding = "CATALOG_CACHE"');
    expect(w).toContain('id = "edb6ca4df12f4f45b40508b3dda3c432"');
    expect(w).not.toMatch(/GEMINI_API_KEY\s*=/);
    expect(w).toContain('wrangler secret put GEMINI_API_KEY');
  });

  it('post134: custom domain route pin', () => {
    const w = read('wrangler.toml');
    expect(w).toContain('pattern = "backlink.fuzzywigg.com"');
    expect(w).toContain('custom_domain = true');
  });

  it('post134: VERSION var pin', () => {
    expect(read('wrangler.toml')).toContain('VERSION = "0.1.0"');
    expect(JSON.parse(read('package.json')).version).toBe('0.1.0');
  });

  it('post134: deploy workflow remains HITL dispatch', () => {
    const d = read('.github/workflows/deploy.yml');
    expect(d).toContain('workflow_dispatch');
    expect(d).not.toMatch(/^\s*push:/m);
    expect(d).not.toMatch(/^\s*pull_request:/m);
  });

  it('post134: existing .cursor/environment.json lock (no thin stubs)', () => {
    const env = read('.cursor/environment.json');
    expect(env).toContain('"name": "Backlink_Facelift"');
    expect(env).toContain('"install": "npm ci"');
    expect(statSync(join(root, '.cursor/environment.json')).size).toBe(57);
  });
  ...
  it('post134: negative inventing fence wrangler', () => {
    expect(read('wrangler.toml')).not.toMatch(/ANTHROPIC|OPENAI|playlist|now-playing/i);
  });
