...
describe('post134 wrangler HEAVY deepen (after #134)', () => {
  ...
  it("post134: HMAC wrangler.toml key post130", () => {
    expect(hmacSha256("post130", "wrangler.toml")).toBe("5aa65c9b5e3020a3c3714840b80c1ce99c3c94dce3decb42198dc0aa2a6a8e3f");
  });

  it("post134: HMAC wrangler.toml key post132", () => {
  ...
  it("post134: HMAC wrangler.toml key leftover-after-#134", () => {
    expect(hmacSha256("leftover-after-#134", "wrangler.toml")).toBe("c6b71f957eeb8b6eb00f1f9f7989f9a89eb9dfec11c9b10d2b066c6a39ad29d1");
  });
  ...
  it("post134: HMAC digests differ for distinct keys on toml", () => {
    ...
    expect(hmacSha256('TOKENMAXX', 'wrangler.toml')).not.toBe(hmacSha256('HEAVY', 'wrangler.toml'));
    expect(hmacSha256('CATALOG_CACHE', 'wrangler.toml')).not.toBe(hmacSha256('VERSION', 'wrangler.toml'));
    expect(hmacSha256('post130', 'wrangler.toml')).not.toBe(hmacSha256('post134', 'wrangler.toml'));
  });

  it("post134: leftover toml forbids envjson/DNS invent in comments", () => {
    expect(toml).not.toMatch(/account_id\s*=/);
    expect(toml).not.toMatch(/api_token/i);
    expect(toml).not.toMatch(/\[env\./);
    expect(toml).toContain('backlink.fuzzywigg.com');
  ...
  it("post134: leftover single KV + single route only", () => {
    expect(toml.match(/\[\[kv_namespaces\]\]/g)).toHaveLength(1);
    expect(toml.match(/\[\[routes\]\]/g)).toHaveLength(1);
    expect([...toml.matchAll(/binding\s*=\s*"([^"]+)"/g)].map((m) => m[1])).toEqual(['CATALOG_CACHE']);
  });
  ...
  it("post134: extra HMAC keys inventory digest", () => {
    const keys = ["wrangler","backlink","CATALOG_CACHE","GEMINI_API_KEY","compatibility_date","kv_namespaces","custom_domain","VERSION","routes","vars","Secrets","fuzzywigg","edb6ca4df12f4f45b40508b3dda3c432","2025-01-01","src/index.ts","post108","post130","post132","HITL","no-invent","leftover-after-#134"];
    expect(createHash('sha256').update(keys.join('|'), 'utf8').digest('hex')).toBe("c4120f6b0741e3cea657be914d1366d072782eefd1aa1bab51ddffe630ae104d");
