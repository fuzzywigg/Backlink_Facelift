...
describe('post149 wrangler HEAVY deepen (after #149)', () => {
  ...
  it("post149: extra HMAC keys inventory digest", () => {
    const keys = ["wrangler","backlink","CATALOG_CACHE","GEMINI_API_KEY","compatibility_date","kv_namespaces","custom_domain","VERSION","routes","vars","Secrets","fuzzywigg","edb6ca4df12f4f45b40508b3dda3c432","2025-01-01","src/index.ts","post108","post126","HITL","no-invent","post134","post149"];
    expect(createHash('sha256').update(keys.join('|'), 'utf8').digest('hex')).toBe("c953a37a90cb96fdd795bdb6135056c9f256930828e8f4879a243fa46f23bba2");
    expect(keys).toHaveLength(21);
  });
  ...
  it("post149: extra HMAC keys inventory digest", () => {
    const keys = ["wrangler","backlink","post149","after-#149","leftover","TOKENMAXX","HEAVY","no-product-invent","no-creds","no-dns-invent","no-history-rewrite","no-envjson-stub","fuzzywigg","iptv-org","CATALOG_CACHE","post140","post141"];
    expect(createHash('sha256').update(keys.join('|'), 'utf8').digest('hex')).toBe("3b2f5720080035a26829c16d90e90ada9cdd38abe18c84088c674f8b20b6a9a9");
    expect(keys).toHaveLength(17);
  });
...
describe('post149 wrangler extras HEAVY deepen (after #149 leftover slice)', () => {
  ...
  it('post149-extras: keys inventory digest', () => {
    const keys = ['post149', 'after-#149', 'leftover', 'TOKENMAXX', 'HEAVY', 'no-product-invent', 'slice-diff', 'no-src-change', 'manifest-lock', 'ci-leftover', 'fuzzywigg', 'backlink', 'CATALOG_CACHE', 'GEMINI_API_KEY', 'iptv-org', 'gemini-2.0-flash', 'VALID_GENRES', 'HITL', 'no-creds', 'station_select'];
    expect(createHash('sha256').update(keys.join('|'), 'utf8').digest('hex')).toBe('19765a0fe3c698f9517c3b8b6427f214fdcdab056b189077a0221cd15a15f628');
  ...
  it('post149-extras: final inventory markers', () => {
    ...
    expect((body.match(/it\('post149-extras:/g) ?? []).length).toBeGreaterThan(400);
  });
});
