...
describe('post144 routes HEAVY deepen (after #144)', () => {
  const read = (rel: string) => readFileSync(join(root, rel), 'utf8');
  const sha256 = (rel: string) => createHash('sha256').update(readFileSync(join(root, rel))).digest('hex');
  const sha1 = (rel: string) => createHash('sha1').update(readFileSync(join(root, rel))).digest('hex');
  const md5 = (rel: string) => createHash('md5').update(readFileSync(join(root, rel))).digest('hex');
  ...
  it('post144: GET / returns service metadata', async () => {
    const res = await app.request('/', undefined, testEnv());
    expect(res.status).toBe(200);
  ...
  it('post144: GET /health ok', async () => {
    const res = await app.request('/health', undefined, testEnv());
    expect(res.status).toBe(200);
  ...
  it('post144: GET /genres lists VALID_GENRES', async () => {
    const res = await app.request('/genres', undefined, testEnv());
    expect(res.status).toBe(200);
  ...
  it('post144: no /playlist or /now-playing product invent', async () => {
    const a = await app.request('/playlist', undefined, testEnv());
    const b = await app.request('/now-playing', undefined, testEnv());
  ...
  it('post144: final inventory markers', () => {
    const body = read('test/routes.test.ts');
    expect(body).toContain("describe('post144 routes HEAVY deepen (after #144)'");
    expect((body.match(/it\('post144:/g) ?? []).length).toBeGreaterThan(60);
  });
});


describe('post144 routes extras HEAVY deepen (after #144 leftover slice)', () => {
  const read = (rel: string) => readFileSync(join(root, rel), 'utf8');
  const sha256 = (rel: string) => createHash('sha256').update(readFileSync(join(root, rel))).digest('hex');
  const sha1 = (rel: string) => createHash('sha1').update(readFileSync(join(root, rel))).digest('hex');
  const md5 = (rel: string) => createHash('md5').update(readFileSync(join(root, rel))).digest('hex');
  ...
  it('post144-extras: inventory digest', () => {
    const keys = ["post144","leftover","TOKENMAXX","HEAVY","after-#144","no-product-invent","slice-diff","no-src-change","manifest-lock","HITL","routes","extras","VALID_GENRES","GENRE_MAP","CATALOG_CACHE","post141"];
    expect(createHash('sha256').update(keys.join('|'), 'utf8').digest('hex')).toBe('a5583f0aa28b629e9a5264230fa36ef25746128fe43247b55476667b2af6933b');
  ...
  it('post144-extras: final markers', () => {
    const body = read('test/routes.test.ts');
    expect(body).toContain("describe('post144 routes HEAVY deepen (after #144)'");
    expect(body).toContain("describe('post144 routes extras HEAVY deepen (after #144 leftover slice)'");
