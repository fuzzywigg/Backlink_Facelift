...
describe('post134 source-contracts HEAVY deepen (after #134)', () => {
  ...
  it("post134: HMAC src/index.ts key post130", () => {
    expect(hmacSha256("post130", "src/index.ts")).toBe("45a27484b87b576c6e7b3e827f78809658ad4a7aaf8b331e1befba2288f5606d");
  });

  it("post134: HMAC src/index.ts key post132", () => {
  ...
  it("post134: HMAC src/index.ts key leftover-after-#134", () => {
    expect(hmacSha256("leftover-after-#134", "src/index.ts")).toBe("466213a27bfc9ce3b1e7453fac7ab0ef7d1e0425f5782106fe7f3aacd0fcad2a");
  });
  ...
  it("post134: resolveGenre(undefined) → music", () => {
    expect(resolveGenre(undefined)).toBe("music");
  });
  ...
  it("post134: leftover envjson stays npm-ci-only (no invent)", () => {
  ...
  it("post134: leftover index does not invent /playlist or /now-playing routes", () => {
    const index = read('src/index.ts');
    expect(index).not.toMatch(/app\.get\('\/playlist'/);
    expect(index).not.toMatch(/app\.get\('\/now-playing'/);
    expect(index).toMatch(/app\.get\('\/curate'/);
  ...
  it("post134: leftover prototype-key resolveGenre edges (no invent)", () => {
    // Mirrors genres suite: __proto__/constructor hit Object.prototype; mixed-case misses.
    ...
    expect(resolveGenre('constructor')).toBe(Object.prototype.constructor);
  ...
  it("post134: extra HMAC keys inventory digest", () => {
    const keys = ["source-contracts","backlink","CATALOG_CACHE","GEMINI_API_KEY","VALID_GENRES","GENRE_MAP","station_select","now_playing","genre_filter","curator_prompt","fuzzywigg","iptv-org","HITL","post111","post130","post132","openapi","VERSION","leftover-after-#134"];
    expect(createHash('sha256').update(keys.join('|'), 'utf8').digest('hex')).toBe("2f2e89ab1e66763dd05e2db5d0624409a30366fc63c025ad85ce179df5c974fb");
