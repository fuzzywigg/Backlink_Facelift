...
describe('post149 source-contracts HEAVY deepen (after #149)', () => {
  ...
  it("post149: extra HMAC keys inventory digest", () => {
    const keys = ["source-contracts","backlink","CATALOG_CACHE","GEMINI_API_KEY","VALID_GENRES","GENRE_MAP","station_select","now_playing","genre_filter","curator_prompt","fuzzywigg","iptv-org","HITL","post111","post126","openapi","VERSION","post134","post149"];
    expect(createHash('sha256').update(keys.join('|'), 'utf8').digest('hex')).toBe("a06db89057936cf18b3e847b6a359fb8c5210ca0bf9240984f012cc1da6d7d47");
    expect(keys).toHaveLength(19);
  });
  ...
  it("post149: extra HMAC keys inventory digest", () => {
    const keys = ["source-contracts","backlink","post149","after-#149","leftover","TOKENMAXX","HEAVY","no-product-invent","no-creds","no-dns-invent","no-history-rewrite","no-envjson-stub","fuzzywigg","iptv-org","CATALOG_CACHE","post140","post141"];
    expect(createHash('sha256').update(keys.join('|'), 'utf8').digest('hex')).toBe("31b7d1cfa90a7b43c237c992c5a088e5ca7f2d1d340e85f63e594e4823f7826d");
    expect(keys).toHaveLength(17);
  });
