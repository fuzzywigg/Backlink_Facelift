...
// --- HEAVY burn (post-#134): deepen source-contracts leftover edges only — no product inventing ---
// Orthogonal to #134 suites. TOKENMAXX tests-only; no credentials/DNS/envjson inventing.
...
describe('post134 source-contracts HEAVY deepen (after #134)', () => {
  ...
  it("post134: extra HMAC keys inventory digest", () => {
    const keys = ["source-contracts","backlink","CATALOG_CACHE","GEMINI_API_KEY","VALID_GENRES","GENRE_MAP","station_select","now_playing","genre_filter","curator_prompt","fuzzywigg","iptv-org","HITL","post111","post134","openapi","VERSION"];
    expect(createHash('sha256').update(keys.join('|'), 'utf8').digest('hex')).toBe("d2e11d33eef3a96b4bedcd8f61814ce947dfe0b7ff7be41ab9b8e51b84e21e60");
    expect(keys).toHaveLength(17);
  });
