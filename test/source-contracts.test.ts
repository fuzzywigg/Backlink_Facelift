...
describe('post144 source-contracts HEAVY deepen (after #144)', () => {
  ...
  it("post144: Gemini 2.0 Flash wired (not Anthropic)", () => {
    const index = read('src/index.ts');
    expect(index).toContain('gemini-2.0-flash');
    expect(index).toContain('generativelanguage.googleapis.com');
    expect(index).not.toMatch(/anthropic|claude|haiku/i);
  ...
  it("post144: iptv-org CDN + KV TTL 3600", () => {
    const index = read('src/index.ts');
    expect(index).toContain('https://iptv-org.github.io/iptv/categories');
    expect(index).toMatch(/expirationTtl:\s*3600/);
  });

  it("post144: Env bindings align with wrangler", () => {
    const types = read('src/types.ts');
    expect(types).toContain('CATALOG_CACHE: KVNamespace');
    expect(types).toMatch(/GEMINI_API_KEY\?:\s*string/);
    expect(types).toMatch(/VERSION\?:\s*string/);
  ...
  it("post144: resolveGenre/GENRE_MAP/VALID_GENRES contracts", () => {
    expect(VALID_GENRES).toHaveLength(9);
    expect(Object.keys(GENRE_MAP)).toHaveLength(21);
    expect(resolveGenre('jazz')).toBe('jazz');
  });
...
describe('post144 source-contracts extras HEAVY deepen (after #144 leftover slice)', () => {
  ...
  it("post144-extras: inventory digest", () => {
    const keys = ["post144","leftover","TOKENMAXX","HEAVY","after-#144","no-product-invent","slice-diff","no-src-change","manifest-lock","HITL","source-contracts","extras","VALID_GENRES","GENRE_MAP","CATALOG_CACHE","post141"];
    expect(createHash('sha256').update(keys.join('|'), 'utf8').digest('hex')).toBe("f79c1778f3d52222e1506b599910554b7b70e531b047357dc39a2507fd67ab80");
