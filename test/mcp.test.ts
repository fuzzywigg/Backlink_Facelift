...
describe('post144 mcp extras HEAVY deepen (after #144 leftover slice)', () => {
  ...
  it("post144-extras: inventory digest", () => {
    const keys = ["post144","leftover","TOKENMAXX","HEAVY","after-#144","no-product-invent","slice-diff","no-src-change","manifest-lock","HITL","mcp","extras","VALID_GENRES","GENRE_MAP","CATALOG_CACHE","post141"];
    expect(createHash('sha256').update(keys.join('|'), 'utf8').digest('hex')).toBe("214f613ce135a5aa92840dc589c30a4a866e33fa8075513bbbb3c3133fefb56c");
    expect(keys).toHaveLength(16);
  });
