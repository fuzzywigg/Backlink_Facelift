...
describe('post144 mcp HEAVY deepen (after #144)', () => {
  ...
  it("post144: final inventory markers", () => {
    ...
    expect((body.match(/it\("post144:/g) ?? []).length).toBeGreaterThan(80);
  });

});

describe('post144 mcp extras HEAVY deepen (after #144 leftover slice)', () => {
  const read = (rel: string) => readFileSync(join(mcpRoot, rel), 'utf8');
  const sha256 = (rel: string) => createHash('sha256').update(readFileSync(join(mcpRoot, rel))).digest('hex');
  const hmacSha256 = (key: string, rel: string) => createHmac('sha256', key).update(readFileSync(join(mcpRoot, rel))).digest('hex');

  it("post144-extras: sha256 src/mcp.ts", () => {
    expect(sha256("src/mcp.ts")).toBe("6ae8ffd7c4b75c471db2dff1fe5c6ad61aff69e38b048a366bb8b7adb3099683");
  });

  it("post144-extras: HMAC post144 src/mcp.ts", () => {
    expect(hmacSha256('post144', "src/mcp.ts")).toBe("aa5c389ea83f1b99ed98e6a832a190b8cdd6a25aa56d776dfaf75a9e781e3b92");
  });

  it("post144-extras: HMAC leftover src/mcp.ts", () => {
    expect(hmacSha256('leftover', "src/mcp.ts")).toBe("8ace2389ce0308341cb978ba9c33116db731b2166d09e0adc79fa37366b8c712");
  });

  it("post144-extras: HMAC TOKENMAXX src/mcp.ts", () => {
    expect(hmacSha256('TOKENMAXX', "src/mcp.ts")).toBe("cc983fd02cae540665f120f8a72c3867b8d573a30f5a9c84ace5b4cea2fb9f62");
  });

  it("post144-extras: HMAC HEAVY src/mcp.ts", () => {
    expect(hmacSha256('HEAVY', "src/mcp.ts")).toBe("870bb2f88c83abd6679e447c6937b01c998a1b3f527024e687dc35c00a046e40");
  });

  it("post144-extras: HMAC after-#144 src/mcp.ts", () => {
    expect(hmacSha256('after-#144', "src/mcp.ts")).toBe("46996564f1d68b91a76d6dbafd853e53872edc499aef2fa8a34dda4a39ebbc38");
  });

  it("post144-extras: HMAC no-product-invent src/mcp.ts", () => {
    expect(hmacSha256('no-product-invent', "src/mcp.ts")).toBe("71a686fe812adcc92f40d5ca19a3996e68555d6749199affd3fc002053aabd65");
  });

  it("post144-extras: HMAC mcp-extras src/mcp.ts", () => {
    expect(hmacSha256('mcp-extras', "src/mcp.ts")).toBe("911991de224dbad0a0561fde149d6d9c5b9c7efe8754b382bd9dea66b27f6fd3");
  });

  it("post144-extras: HMAC slice-diff src/mcp.ts", () => {
    expect(hmacSha256('slice-diff', "src/mcp.ts")).toBe("4f4c0995596fa6a8eecfa27d7f7047b084074e0acf00d6890811a88f5f807e05");
  });

  it("post144-extras: HMAC no-src-change src/mcp.ts", () => {
    expect(hmacSha256('no-src-change', "src/mcp.ts")).toBe("83880bdeca0f5e263ef18761f18cac15707d298b1c7fe1157e766221d37f343a");
  });

  it("post144-extras: HMAC manifest-lock src/mcp.ts", () => {
    expect(hmacSha256('manifest-lock', "src/mcp.ts")).toBe("b87b5d8230d2c0d160c1d3fd8e6864b8344b3765e2a3d5620130f45110a6c503");
  });

  it("post144-extras: sha256 docs/mcp-spec.md", () => {
    expect(sha256("docs/mcp-spec.md")).toBe("a93978d779b976a1aba4d34395eec7628b21bda910ef8a279d5efbc05da56849");
  });

  it("post144-extras: HMAC post144 docs/mcp-spec.md", () => {
    expect(hmacSha256('post144', "docs/mcp-spec.md")).toBe("aff70172217fde98f5456142175cebca2967f1ac505d1a07aad66f3677b6407f");
  });

  it("post144-extras: HMAC leftover docs/mcp-spec.md", () => {
    expect(hmacSha256('leftover', "docs/mcp-spec.md")).toBe("cc7b82d7e2cbbb55051894ddba60fbf4023572b4cd7a21b98ebf76001a5d07df");
  });

  it("post144-extras: HMAC TOKENMAXX docs/mcp-spec.md", () => {
    expect(hmacSha256('TOKENMAXX', "docs/mcp-spec.md")).toBe("58bd8b12de8084ece067f68db2cea2ea5dc8b43305c0d5e7e20506fd40e18749");
  });

  it("post144-extras: HMAC HEAVY docs/mcp-spec.md", () => {
    expect(hmacSha256('HEAVY', "docs/mcp-spec.md")).toBe("14502ba27898e795fb59cc37b06fe6eb2a66b40811b068e87738eb3dbf4cae59");
  });

  it("post144-extras: HMAC after-#144 docs/mcp-spec.md", () => {
    expect(hmacSha256('after-#144', "docs/mcp-spec.md")).toBe("15be4839db21adb45403babff19f28198cd6339284bc698326e8f31e017809cf");
  });

  it("post144-extras: HMAC no-product-invent docs/mcp-spec.md", () => {
    expect(hmacSha256('no-product-invent', "docs/mcp-spec.md")).toBe("1ea2c93af05439c6a1c14acfbee6d34dddfc2b32e6c8b6256c270f6d4484b608");
  });

  it("post144-extras: HMAC mcp-extras docs/mcp-spec.md", () => {
    expect(hmacSha256('mcp-extras', "docs/mcp-spec.md")).toBe("d52043b6f3a4fcbfc8eea0901a03adcfe973a4f1d7e455a3e0cf154b5d938bef");
  });

  it("post144-extras: HMAC slice-diff docs/mcp-spec.md", () => {
    expect(hmacSha256('slice-diff', "docs/mcp-spec.md")).toBe("06d50d14b3b548000878558498c74926788485f641f279bbbdfe5d14f0bbbb4a");
  });

  it("post144-extras: HMAC no-src-change docs/mcp-spec.md", () => {
    expect(hmacSha256('no-src-change', "docs/mcp-spec.md")).toBe("84ca704115fb25233f9bf1ba5e611bdd463e200ae837d762c61d4280a26ba6db");
  });

  it("post144-extras: HMAC manifest-lock docs/mcp-spec.md", () => {
    expect(hmacSha256('manifest-lock', "docs/mcp-spec.md")).toBe("b70a770cd09ea94dcd51d690d3a96822867773eae92d2afc16ef212ee8537513");
  });

  it("post144-extras: sha256 package.json", () => {
    expect(sha256("package.json")).toBe("34552493f3008b58991d10e7b41ee0ecaa43bf8ba3e79d261ac2a061e6f7181c");
  });

  it("post144-extras: HMAC post144 package.json", () => {
    expect(hmacSha256('post144', "package.json")).toBe("d135fe4b6b5fdc18e139c6b5f6519ab396435023f315e2dd00371db2100b73f5");
  });

  it("post144-extras: HMAC leftover package.json", () => {
    expect(hmacSha256('leftover', "package.json")).toBe("20e0c5771e324d5d7c4d9bb108e54226b1ca026d3c6d232d5f0b8ccba88462a1");
  });

  it("post144-extras: HMAC TOKENMAXX package.json", () => {
    expect(hmacSha256('TOKENMAXX', "package.json")).toBe("ff224f52701ef6f2ee2609bc2bd5cdf346a14ef6b4b5eab51bbf86a8b01bca58");
  });

  it("post144-extras: HMAC HEAVY package.json", () => {
    expect(hmacSha256('HEAVY', "package.json")).toBe("59f02fb62823abdd3ebccdd68ef1f27db9333e414f49a111c132eca85acb6563");
  });

  it("post144-extras: HMAC after-#144 package.json", () => {
    expect(hmacSha256('after-#144', "package.json")).toBe("3639b440b8a35b7453a36a1d2c6085f07314888363f235ee186b981606dd242a");
  });

  it("post144-extras: HMAC no-product-invent package.json", () => {
    expect(hmacSha256('no-product-invent', "package.json")).toBe("b4d2e3db95a68120d3e5f1dc0b35bda72e5a8ffa0c34dd3b2b110699c0cd286b");
  });

  it("post144-extras: HMAC mcp-extras package.json", () => {
    expect(hmacSha256('mcp-extras', "package.json")).toBe("0ff8843ab2f9cd380e8ff37671c986466431eb143efd2c9fec27a792ebe1df9f");
  });

  it("post144-extras: HMAC slice-diff package.json", () => {
    expect(hmacSha256('slice-diff', "package.json")).toBe("dc869038c3869410db214fb5618c8f6091545ded44463dd4ccdf26cefc3dca56");
  });

  it("post144-extras: HMAC no-src-change package.json", () => {
    expect(hmacSha256('no-src-change', "package.json")).toBe("f89ef881d4621b0b9716e0341fb3cd014b14115046ec8e1fbc1baa995c764959");
  });

  it("post144-extras: HMAC manifest-lock package.json", () => {
    expect(hmacSha256('manifest-lock', "package.json")).toBe("c78857efdc2a9ef7366350439061383f242d1056da2d338f56f4d698b5748417");
  });

  it("post144-extras: sha256 AGENTS.md", () => {
    expect(sha256("AGENTS.md")).toBe("48e590b4f146e2fbd1ebb409e0d5a5ec1be50b72b2c310f1c1e360487b36feaa");
  });

  it("post144-extras: HMAC post144 AGENTS.md", () => {
    expect(hmacSha256('post144', "AGENTS.md")).toBe("305dcaeab8aff3091db106c83ea5389ef3b11783dd50610eb804169a6bf66c34");
  });

  it("post144-extras: HMAC leftover AGENTS.md", () => {
    expect(hmacSha256('leftover', "AGENTS.md")).toBe("ebc9f95bcc289e29e0a1ef806d4a6466da053e934eba9da783fda10f1a46b84e");
  });

  it("post144-extras: HMAC TOKENMAXX AGENTS.md", () => {
    expect(hmacSha256('TOKENMAXX', "AGENTS.md")).toBe("b3fb6ac3a6100a53c55b09762041608ae8003dd239b191726b2de0f18ae2b72f");
  });

  it("post144-extras: HMAC HEAVY AGENTS.md", () => {
    expect(hmacSha256('HEAVY', "AGENTS.md")).toBe("f5534ae49c23be34018c9e05a44b201edf94a776bd06b184d44b41e02e77c87c");
  });

  it("post144-extras: HMAC after-#144 AGENTS.md", () => {
    expect(hmacSha256('after-#144', "AGENTS.md")).toBe("645d38df46b86bae358d6e5fc6e743303c62fde04185b839f46a5c0b46bff308");
  });

  it("post144-extras: HMAC no-product-invent AGENTS.md", () => {
    expect(hmacSha256('no-product-invent', "AGENTS.md")).toBe("dbfdb45d097dffeee56f94781c4ce33e6c8cfb185871bf00c7237385c42cf264");
  });

  it("post144-extras: HMAC mcp-extras AGENTS.md", () => {
    expect(hmacSha256('mcp-extras', "AGENTS.md")).toBe("42ea8cc769813087c223b8b1a055ffa912d6b731e6f8d6bc6b947d6b03a57750");
  });

  it("post144-extras: HMAC slice-diff AGENTS.md", () => {
    expect(hmacSha256('slice-diff', "AGENTS.md")).toBe("870cb0b7ca943c82a943cb231e6171ba55b8a1be90d4368fe1e30a3666060c90");
  });

  it("post144-extras: HMAC no-src-change AGENTS.md", () => {
    expect(hmacSha256('no-src-change', "AGENTS.md")).toBe("94cb89045ff7e09a81a02dd8ee4ee7eb0ef7d412d7260f9ab24056774ed00967");
  });

  it("post144-extras: HMAC manifest-lock AGENTS.md", () => {
    expect(hmacSha256('manifest-lock', "AGENTS.md")).toBe("af3b8d7bad009043c764c1d1f67b9526650ba158a9053d887985ddce5dc997c5");
  });

  it("post144-extras: sha256 vitest.config.ts", () => {
    expect(sha256("vitest.config.ts")).toBe("f9b58bb937531da55ad474592e69ec95c6d55a5b8b878f8fa251c0f8d6caff38");
  });

  it("post144-extras: HMAC post144 vitest.config.ts", () => {
    expect(hmacSha256('post144', "vitest.config.ts")).toBe("b40fa27df2e649db712b9e17fd2883d3c92c3e1e59e0ce09b5247fc908fcd883");
  });

  it("post144-extras: HMAC leftover vitest.config.ts", () => {
    expect(hmacSha256('leftover', "vitest.config.ts")).toBe("3bc8abcf1f58dc77ee233f74f3e725de7089ea5307ef488f25b1aad2d0f3d1b7");
  });

  it("post144-extras: HMAC TOKENMAXX vitest.config.ts", () => {
    expect(hmacSha256('TOKENMAXX', "vitest.config.ts")).toBe("0f446a2e20693c7657cb1d718f1a1b296160af17a69fcd36cec18d937ae65de9");
  });

  it("post144-extras: HMAC HEAVY vitest.config.ts", () => {
    expect(hmacSha256('HEAVY', "vitest.config.ts")).toBe("08ec43359860bb937405b1b476b372ee74b0d49b19430497c923df04bbe60179");
  });

  it("post144-extras: HMAC after-#144 vitest.config.ts", () => {
    expect(hmacSha256('after-#144', "vitest.config.ts")).toBe("bfcb2e6995eb5a0f1b3fe347cdec5528923ff11648c3869661b8d13a5f1500bd");
  });

  it("post144-extras: HMAC no-product-invent vitest.config.ts", () => {
    expect(hmacSha256('no-product-invent', "vitest.config.ts")).toBe("3e3b5178103ca33942111d45dcf7e812cb38dc01558a23497b43440560df420c");
  });

  it("post144-extras: HMAC mcp-extras vitest.config.ts", () => {
    expect(hmacSha256('mcp-extras', "vitest.config.ts")).toBe("9a07cdae561aadb319469efd9b12db1c64362d61ea0de2b0e7acc5fa5a9f0178");
  });

  it("post144-extras: HMAC slice-diff vitest.config.ts", () => {
    expect(hmacSha256('slice-diff', "vitest.config.ts")).toBe("561ba73465a9de5fd03d403c4dd508c2058439a0206f6a694190dd71b47a4f70");
  });

  it("post144-extras: HMAC no-src-change vitest.config.ts", () => {
    expect(hmacSha256('no-src-change', "vitest.config.ts")).toBe("9e2fe808f40ed7a1049bef62978dd65efbea705482829585570b3fc4f5bf1a54");
  });

  it("post144-extras: HMAC manifest-lock vitest.config.ts", () => {
    expect(hmacSha256('manifest-lock', "vitest.config.ts")).toBe("7ae3b954bcee884ec6b1aada9b98e3b746465b99d10627ccb8def98e1e97799d");
  });

  it("post144-extras: sha256 src/genres.ts", () => {
    expect(sha256("src/genres.ts")).toBe("aa626817cf3bc8a707ac5adba39f811dfbc23f695e5e0cb9d070007d839d914e");
  });

  it("post144-extras: HMAC post144 src/genres.ts", () => {
    expect(hmacSha256('post144', "src/genres.ts")).toBe("deaeba77d3cede8aa31901f4296c4e799da0d9a41ce5a2956f0bdd43a69633d5");
  });

  it("post144-extras: HMAC leftover src/genres.ts", () => {
    expect(hmacSha256('leftover', "src/genres.ts")).toBe("bc49b5523a2b049ad6ea2abd27732f8e53d9ff48d9296f3da0bd9cb76d72624f");
  });

  it("post144-extras: HMAC TOKENMAXX src/genres.ts", () => {
    expect(hmacSha256('TOKENMAXX', "src/genres.ts")).toBe("7abd1ff098bb19d7a63a4b6c2c44965f0ea04ceb5fc40896c92ef7ea8f00b951");
  });

  it("post144-extras: HMAC HEAVY src/genres.ts", () => {
    expect(hmacSha256('HEAVY', "src/genres.ts")).toBe("728dd3fe7c4667ea4d489028dc3a100c092d2ede6b7319716769186532d3b575");
  });

  it("post144-extras: HMAC after-#144 src/genres.ts", () => {
    expect(hmacSha256('after-#144', "src/genres.ts")).toBe("709b9688780c0634269e4e70a900a83bae3b5406c1051cbfc3215e4c577ef762");
  });

  it("post144-extras: HMAC no-product-invent src/genres.ts", () => {
    expect(hmacSha256('no-product-invent', "src/genres.ts")).toBe("3d21ae09929f61fc420c1aff78e7fbcdaa55895034e581f9845399de2569142b");
  });

  it("post144-extras: HMAC mcp-extras src/genres.ts", () => {
    expect(hmacSha256('mcp-extras', "src/genres.ts")).toBe("d0261cc510ac443616f0dad5b5382934121970bfd754c19e95461c28cc098542");
  });

  it("post144-extras: HMAC slice-diff src/genres.ts", () => {
    expect(hmacSha256('slice-diff', "src/genres.ts")).toBe("22d66e269afda39844782f2607b23bacf23e5d02a51249ddec437782bc1a3a78");
  });

  it("post144-extras: HMAC no-src-change src/genres.ts", () => {
    expect(hmacSha256('no-src-change', "src/genres.ts")).toBe("a3b9a583e340975667c401c4933eb3535590637cdc49d496ffec24efecd28810");
  });

  it("post144-extras: HMAC manifest-lock src/genres.ts", () => {
    expect(hmacSha256('manifest-lock', "src/genres.ts")).toBe("e9f5e2172614a8e3160d35040a7380c6d239a34a24fe0b257862f3edc83bb4dc");
  });

  it("post144-extras: sha256 src/index.ts", () => {
    expect(sha256("src/index.ts")).toBe("7f0d574b0aedc6cd71d3ea35bb03e2a20389028e6ff2c195718acff2e0313a72");
  });

  it("post144-extras: HMAC post144 src/index.ts", () => {
    expect(hmacSha256('post144', "src/index.ts")).toBe("193c749dc85e2745d03b41be50db7671e0e1f8d1147558d564cc771835e3a685");
  });

  it("post144-extras: HMAC leftover src/index.ts", () => {
    expect(hmacSha256('leftover', "src/index.ts")).toBe("268d5e353fd881bdd119b1f654cb291896d509e3f70768fde43a2bef6fdea2be");
  });

  it("post144-extras: HMAC TOKENMAXX src/index.ts", () => {
    expect(hmacSha256('TOKENMAXX', "src/index.ts")).toBe("d25579a5c0d84b104f95ce77a95b760199b110e6ac8ae500fbfbda0c904e7cdc");
  });

  it("post144-extras: HMAC HEAVY src/index.ts", () => {
    expect(hmacSha256('HEAVY', "src/index.ts")).toBe("f3d8136884b78d12b0d57975d091081c2234daac8b42ecdabbf37d8bb6022b30");
  });

  it("post144-extras: HMAC after-#144 src/index.ts", () => {
    expect(hmacSha256('after-#144', "src/index.ts")).toBe("d83fda242e36a57a5a31d57c22f0a33c7aa00c054f64a0817e90be63c9548d0c");
  });

  it("post144-extras: HMAC no-product-invent src/index.ts", () => {
    expect(hmacSha256('no-product-invent', "src/index.ts")).toBe("49e017c3ff39fee9c0f5d47c30cccb692dd0c4c39f3d36655bbb08108fcc7e0a");
  });

  it("post144-extras: HMAC mcp-extras src/index.ts", () => {
    expect(hmacSha256('mcp-extras', "src/index.ts")).toBe("3a8ded688d018954c827ef7444c16946916529eff18573b2cacdbfea998d5980");
  });

  it("post144-extras: HMAC slice-diff src/index.ts", () => {
    expect(hmacSha256('slice-diff', "src/index.ts")).toBe("c2be27a92bce9fa17108fbf90f63b917d25d4114397192e04caf79ac29e8f44f");
  });

  it("post144-extras: HMAC no-src-change src/index.ts", () => {
    expect(hmacSha256('no-src-change', "src/index.ts")).toBe("f1b32cf267f0c31a79062ca85a6243b3f166c1d5dcf534c3149a97e9d6ae4846");
  });

  it("post144-extras: HMAC manifest-lock src/index.ts", () => {
    expect(hmacSha256('manifest-lock', "src/index.ts")).toBe("41e4be551225bde732f63c07d1a0d8c24bd630a5c649a3b4c3eb88ca0338c946");
  });

  it("post144-extras: sha256 wrangler.toml", () => {
    expect(sha256("wrangler.toml")).toBe("95b11779a88f0544f3561eea67994a0b0b874d7b8776579189fa7142fa0473f8");
  });

  it("post144-extras: HMAC post144 wrangler.toml", () => {
    expect(hmacSha256('post144', "wrangler.toml")).toBe("9499d8e6f8f7a4c1eed6ba5eabc05e7d0e24609e96dd23cefbb3a169ab766560");
  });

  it("post144-extras: HMAC leftover wrangler.toml", () => {
    expect(hmacSha256('leftover', "wrangler.toml")).toBe("117043293c91e6cdcad8f44181f5c253ceb0f7dc567ea32cddbd61f9d349a063");
  });

  it("post144-extras: HMAC TOKENMAXX wrangler.toml", () => {
    expect(hmacSha256('TOKENMAXX', "wrangler.toml")).toBe("7d198a7e11f32e841079eb2398433044d49d9336bb0642737d55dbb39a1206d4");
  });

  it("post144-extras: HMAC HEAVY wrangler.toml", () => {
    expect(hmacSha256('HEAVY', "wrangler.toml")).toBe("0106e385ea2e0ca3fd52ddc940a1eb5921a22885362d57f5a49dd1bda89ea5db");
  });

  it("post144-extras: HMAC after-#144 wrangler.toml", () => {
    expect(hmacSha256('after-#144', "wrangler.toml")).toBe("d4e996551dc8a7aaed7067a46cd0d50729765f745fa7487535d8f7e2cd04cab8");
  });

  it("post144-extras: HMAC no-product-invent wrangler.toml", () => {
    expect(hmacSha256('no-product-invent', "wrangler.toml")).toBe("3d99d134e0673c8ff163b29a6c72e49bfa5e599898762bf47dd20f0e65639abb");
  });

  it("post144-extras: HMAC mcp-extras wrangler.toml", () => {
    expect(hmacSha256('mcp-extras', "wrangler.toml")).toBe("d28c38ed34812c9091c9de54e7e010626923f00f2d204736e559d7add9791a2c");
  });

  it("post144-extras: HMAC slice-diff wrangler.toml", () => {
    expect(hmacSha256('slice-diff', "wrangler.toml")).toBe("ff10d155b4bfa2137b3d8248b28954f776f80f51ddf29915f4a0792dca9e4b17");
  });

  it("post144-extras: HMAC no-src-change wrangler.toml", () => {
    expect(hmacSha256('no-src-change', "wrangler.toml")).toBe("b542f61e71ea970554a5d78c6f3caa0491697694218687f89822c8cc8509b67b");
  });

  it("post144-extras: HMAC manifest-lock wrangler.toml", () => {
    expect(hmacSha256('manifest-lock', "wrangler.toml")).toBe("ba1c46086c0d7c2f08a90b091ba228b20b7bf50c702c3e3478a8b4b96e95d882");
  });

  it("post144-extras: tool count 4", () => { expect(MCP_MANIFEST.tools).toHaveLength(4); });
  it("post144-extras: schema_version v1", () => { expect(MCP_MANIFEST.schema_version).toBe("v1"); });
  it("post144-extras: name_for_model backlink", () => { expect(MCP_MANIFEST.name_for_model).toBe("backlink"); });
  it("post144-extras: name_for_human Backlink Radio", () => { expect(MCP_MANIFEST.name_for_human).toBe("Backlink Radio"); });
  it("post144-extras: auth none", () => { expect(MCP_MANIFEST.auth).toEqual({ type: "none" }); });
  it("post144-extras: api openapi", () => { expect(MCP_MANIFEST.api).toEqual({ type: "openapi", url: "/openapi.json" }); });
  it("post144-extras: tool station_select present", () => { expect(MCP_MANIFEST.tools.map(x => x.name)).toContain("station_select"); });
  it("post144-extras: tool station_select no claw_", () => { expect("station_select".startsWith("claw_")).toBe(false); });
  it("post144-extras: tool now_playing present", () => { expect(MCP_MANIFEST.tools.map(x => x.name)).toContain("now_playing"); });
  it("post144-extras: tool now_playing no claw_", () => { expect("now_playing".startsWith("claw_")).toBe(false); });
  it("post144-extras: tool genre_filter present", () => { expect(MCP_MANIFEST.tools.map(x => x.name)).toContain("genre_filter"); });
  it("post144-extras: tool genre_filter no claw_", () => { expect("genre_filter".startsWith("claw_")).toBe(false); });
  it("post144-extras: tool curator_prompt present", () => { expect(MCP_MANIFEST.tools.map(x => x.name)).toContain("curator_prompt"); });
  it("post144-extras: tool curator_prompt no claw_", () => { expect("curator_prompt".startsWith("claw_")).toBe(false); });
  it("post144-extras: mcp.ts forbids /playlist", () => {
    expect(read("src/mcp.ts").toLowerCase()).not.toContain("/playlist");
  });
  it("post144-extras: mcp.ts forbids /now-playing endpoint", () => {
    expect(read("src/mcp.ts").toLowerCase()).not.toContain("/now-playing endpoint");
  });
  it("post144-extras: mcp.ts forbids workers.ai", () => {
    expect(read("src/mcp.ts").toLowerCase()).not.toContain("workers.ai");
  });
  it("post144-extras: mcp.ts forbids d1_", () => {
    expect(read("src/mcp.ts").toLowerCase()).not.toContain("d1_");
  });
  it("post144-extras: mcp.ts forbids r2_", () => {
    expect(read("src/mcp.ts").toLowerCase()).not.toContain("r2_");
  });
  it("post144-extras: mcp.ts forbids vectorize", () => {
    expect(read("src/mcp.ts").toLowerCase()).not.toContain("vectorize");
  });
  it("post144-extras: mcp.ts forbids analytics engine", () => {
    expect(read("src/mcp.ts").toLowerCase()).not.toContain("analytics engine");
  });
  it("post144-extras: mcp.ts forbids hyperdrive", () => {
    expect(read("src/mcp.ts").toLowerCase()).not.toContain("hyperdrive");
  });
  it("post144-extras: mcp.ts forbids anthropic", () => {
    expect(read("src/mcp.ts").toLowerCase()).not.toContain("anthropic");
  });
  it("post144-extras: mcp.ts forbids claude", () => {
    expect(read("src/mcp.ts").toLowerCase()).not.toContain("claude");
  });
  it("post144-extras: mcp.ts forbids openai", () => {
    expect(read("src/mcp.ts").toLowerCase()).not.toContain("openai");
  });
  it("post144-extras: mcp.ts forbids durable_object", () => {
    expect(read("src/mcp.ts").toLowerCase()).not.toContain("durable_object");
  });
  it("post144-extras: mcp.ts forbids queue_", () => {
    expect(read("src/mcp.ts").toLowerCase()).not.toContain("queue_");
  });
  it("post144-extras: mcp.ts forbids service binding", () => {
    expect(read("src/mcp.ts").toLowerCase()).not.toContain("service binding");
  });
  it("post144-extras: mcp.ts forbids ANTHROPIC_API_KEY", () => {
    expect(read("src/mcp.ts").toLowerCase()).not.toContain("anthropic_api_key");
  });
  it("post144-extras: mcp.ts forbids OPENAI_API_KEY", () => {
    expect(read("src/mcp.ts").toLowerCase()).not.toContain("openai_api_key");
  });
  it("post144-extras: compact HMAC post144", () => {
    expect(createHmac('sha256', "post144").update(JSON.stringify(MCP_MANIFEST), 'utf8').digest('hex')).toBe("125455fbf748738b7b72f877abc6e9faf34e7a9904c21332877a293b9ae5a361");
  });
  it("post144-extras: compact HMAC leftover", () => {
    expect(createHmac('sha256', "leftover").update(JSON.stringify(MCP_MANIFEST), 'utf8').digest('hex')).toBe("6503ddae3d9c2d52d19c2828c8d98b979a5afe735094369937c01437b4993409");
  });
  it("post144-extras: compact HMAC TOKENMAXX", () => {
    expect(createHmac('sha256', "TOKENMAXX").update(JSON.stringify(MCP_MANIFEST), 'utf8').digest('hex')).toBe("ac3aa328b66887e37f2a98c5b73872089735c9150d4a3d4c00192b9e1974fbac");
  });
  it("post144-extras: compact HMAC HEAVY", () => {
    expect(createHmac('sha256', "HEAVY").update(JSON.stringify(MCP_MANIFEST), 'utf8').digest('hex')).toBe("360825dd0456638ccca74ad826aae66f7f2f91f3303154f77e77ac5bc76ef27f");
  });
  it("post144-extras: compact HMAC after-#144", () => {
    expect(createHmac('sha256', "after-#144").update(JSON.stringify(MCP_MANIFEST), 'utf8').digest('hex')).toBe("46c8f9dbca8021f64a04b13060b2db734d2a9c36307581ce7fd889dbdebcea9b");
  });
  it("post144-extras: compact HMAC no-product-invent", () => {
    expect(createHmac('sha256', "no-product-invent").update(JSON.stringify(MCP_MANIFEST), 'utf8').digest('hex')).toBe("1ec4402a90b3f87fb1fd32de33122b40fedaa3a1d623804b31bbd11109100713");
  });
  it("post144-extras: compact HMAC mcp-extras", () => {
    expect(createHmac('sha256', "mcp-extras").update(JSON.stringify(MCP_MANIFEST), 'utf8').digest('hex')).toBe("c31c66b8bc519baac89821e3207783700fcefeb5e2ac8468e2591c6c9bf6206e");
  });
  it("post144-extras: compact HMAC slice-diff", () => {
    expect(createHmac('sha256', "slice-diff").update(JSON.stringify(MCP_MANIFEST), 'utf8').digest('hex')).toBe("063691ca5879ebcb498de8dc0cef313b59d25974b67ae956c77c57961be05875");
  });
  it("post144-extras: compact HMAC no-src-change", () => {
    expect(createHmac('sha256', "no-src-change").update(JSON.stringify(MCP_MANIFEST), 'utf8').digest('hex')).toBe("d21948b252ebfa0e6fcae26409cc12f483b26791282c1f0a316f78a0d46b25bb");
  });
  it("post144-extras: compact HMAC manifest-lock", () => {
    expect(createHmac('sha256', "manifest-lock").update(JSON.stringify(MCP_MANIFEST), 'utf8').digest('hex')).toBe("8ded0ef0603cba9cfa775a5a3b9c558d0a4820bce8c3ca0d1a54dcb1e1e0829e");
  });
  it("post144-extras: mega purity 50x mcp.ts", () => {
    const expected = "6ae8ffd7c4b75c471db2dff1fe5c6ad61aff69e38b048a366bb8b7adb3099683";
    for (let i = 0; i < 50; i++) expect(sha256("src/mcp.ts")).toBe(expected);
  });
  it("post144-extras: inventory digest", () => {
    const keys = ["post144","leftover","TOKENMAXX","HEAVY","after-#144","no-product-invent","mcp-extras","slice-diff","no-src-change","manifest-lock","station_select","now_playing","genre_filter","curator_prompt","post140","post144","extras"];
    expect(createHash('sha256').update(keys.join('|'), 'utf8').digest('hex')).toBe("a5babf035aa3b1a3183fccf6ed06995855ad425f28ac1106cd19c5a6c753e65c");
    expect(keys).toHaveLength(17);
  });
  it("post144-extras: final markers", () => {
    const body = read('test/mcp.test.ts');
    expect(body).toContain("describe('post144 mcp HEAVY deepen (after #144)'");
    expect(body).toContain("describe('post144 mcp extras HEAVY deepen (after #144 leftover slice)'");
