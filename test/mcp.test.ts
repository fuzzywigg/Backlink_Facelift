...
describe('post149 mcp HEAVY deepen (after #149)', () => {
  ...
  it("post149: extra HMAC keys inventory digest", () => {
    const keys = ["mcp","backlink","Backlink Radio","station_select","now_playing","genre_filter","curator_prompt","openapi","schema_version","claw-mcp","fuzzywigg","iptv-org","post108","post126","VALID_GENRES","GENRE_MAP","post134","post149"];
    expect(createHash('sha256').update(keys.join('|'), 'utf8').digest('hex')).toBe("e01b813458428d61fa96218af9336543d9297897ed05e4a7c311011c5ca87a64");
    expect(keys).toHaveLength(18);
  });
  ...
  it("post149: extra HMAC keys inventory digest", () => {
    const keys = ["mcp","backlink","post149","after-#149","leftover","TOKENMAXX","HEAVY","no-product-invent","no-creds","no-dns-invent","no-history-rewrite","no-envjson-stub","fuzzywigg","iptv-org","CATALOG_CACHE","post140","post141"];
    expect(createHash('sha256').update(keys.join('|'), 'utf8').digest('hex')).toBe("d5f93a758122701dcf9d80dbb27606b80b48e74c471ee9f4d8b6cde6f75cfc60");
    expect(keys).toHaveLength(17);
  });
