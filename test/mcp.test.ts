...
describe('post134 mcp HEAVY deepen (after #134)', () => {
  ...
  it("post134: HMAC src/mcp.ts key post130", () => {
    expect(hmacSha256("post130", "src/mcp.ts")).toBe("12a1c6a4b2671a5b77c82037e11077b6617ebd6d5d1067c38f7265586cd53749");
  });

  it("post134: HMAC src/mcp.ts key post132", () => {
  ...
  it("post134: HMAC src/mcp.ts key leftover-after-#134", () => {
    expect(hmacSha256("leftover-after-#134", "src/mcp.ts")).toBe("a5f67a279825cec94db53785ddcb1d072eca254eb6716a5c900a0e6120f90032");
  });
  ...
  it("post134: compact HMAC key post130", () => {
    expect(createHmac('sha256', "post130").update(JSON.stringify(MCP_MANIFEST), 'utf8').digest('hex')).toBe("bc6bc3f69af8de37dd478ca662d64c41f19f115cdc3ca66d1b7b6b674fd5c1c8");
  });

  it("post134: compact HMAC key post132", () => {
    expect(createHmac('sha256', "post132").update(JSON.stringify(MCP_MANIFEST), 'utf8').digest('hex')).toBe("34f7d3541247f4270ba778d0a1c977f77d9de9d127ceb1738f708331564f124a");
  });
  ...
  it("post134: src/mcp.ts forbids invent phrase /playlist", () => {
    expect(read("src/mcp.ts").toLowerCase()).not.toContain("/playlist");
  });

  it("post134: src/mcp.ts forbids invent phrase /now-playing endpoint", () => {
    expect(read("src/mcp.ts").toLowerCase()).not.toContain("/now-playing endpoint");
  });

  it("post134: src/mcp.ts forbids invent phrase workers ai", () => {
    expect(read("src/mcp.ts").toLowerCase()).not.toContain("workers ai");
  });

  it("post134: src/mcp.ts forbids invent phrase d1_", () => {
    expect(read("src/mcp.ts").toLowerCase()).not.toContain("d1_");
  });

  it("post134: src/mcp.ts forbids invent phrase r2_", () => {
    expect(read("src/mcp.ts").toLowerCase()).not.toContain("r2_");
  });

  it("post134: src/mcp.ts forbids invent phrase vectorize", () => {
    expect(read("src/mcp.ts").toLowerCase()).not.toContain("vectorize");
  });

  it("post134: src/mcp.ts forbids invent phrase analytics engine", () => {
    expect(read("src/mcp.ts").toLowerCase()).not.toContain("analytics engine");
  });

  it("post134: src/mcp.ts forbids invent phrase hyperdrive", () => {
    expect(read("src/mcp.ts").toLowerCase()).not.toContain("hyperdrive");
  });

  it("post134: src/mcp.ts forbids invent phrase anthropic", () => {
    expect(read("src/mcp.ts").toLowerCase()).not.toContain("anthropic");
  });

  it("post134: src/mcp.ts forbids invent phrase claude", () => {
    expect(read("src/mcp.ts").toLowerCase()).not.toContain("claude");
  });

  it("post134: src/mcp.ts forbids invent phrase openai", () => {
    expect(read("src/mcp.ts").toLowerCase()).not.toContain("openai");
  });

  it("post134: src/mcp.ts forbids invent phrase durable_object", () => {
    expect(read("src/mcp.ts").toLowerCase()).not.toContain("durable_object");
  });

  it("post134: src/mcp.ts forbids invent phrase queue_", () => {
    expect(read("src/mcp.ts").toLowerCase()).not.toContain("queue_");
  });

  it("post134: src/mcp.ts forbids invent phrase service binding", () => {
    expect(read("src/mcp.ts").toLowerCase()).not.toContain("service binding");
  });
  ...
  it("post134: HMAC digests differ for distinct keys on mcp.ts", () => {
    ...
    expect(hmacSha256('TOKENMAXX', 'src/mcp.ts')).not.toBe(hmacSha256('HEAVY', 'src/mcp.ts'));
    expect(hmacSha256('station_select', 'src/mcp.ts')).not.toBe(hmacSha256('now_playing', 'src/mcp.ts'));
    expect(hmacSha256('post130', 'src/mcp.ts')).not.toBe(hmacSha256('post134', 'src/mcp.ts'));
  });

  it("post134: leftover timingSafeEqual on mcp.ts sha256", () => {
    const a = Buffer.from(sha256('src/mcp.ts'), 'hex');
    const b = Buffer.from("6ae8ffd7c4b75c471db2dff1fe5c6ad61aff69e38b048a366bb8b7adb3099683", 'hex');
    expect(timingSafeEqual(a, b)).toBe(true);
  });

  it("post134: leftover schema_version v1 only (no invent)", () => {
    expect(MCP_MANIFEST.schema_version).toBe('v1');
    expect(MCP_MANIFEST.schema_version).not.toBe('v2');
    expect(Object.keys(MCP_MANIFEST).sort()).toEqual([
      'api', 'auth', 'description_for_human', 'description_for_model',
      'name_for_human', 'name_for_model', 'schema_version', 'tools',
    ]);
  });

  it("post134: leftover tools never invent /health as a claw-mcp tool", () => {
    expect(MCP_MANIFEST.tools.map((t) => t.name)).not.toContain('health');
    expect(MCP_MANIFEST.tools.map((t) => t.name)).not.toContain('backlink_health');
    expect(MCP_MANIFEST.tools.map((t) => t.name)).not.toContain('playlist');
  });

  it("post134: extra HMAC keys inventory digest", () => {
    const keys = ["mcp","backlink","Backlink Radio","station_select","now_playing","genre_filter","curator_prompt","openapi","schema_version","claw-mcp","fuzzywigg","iptv-org","post108","post130","post132","VALID_GENRES","GENRE_MAP","leftover-after-#134"];
    expect(createHash('sha256').update(keys.join('|'), 'utf8').digest('hex')).toBe("e3a84ae89a8b55a866ea90fa9d15a43a92c01a8de12ae2f43895877b9022fbf3");
