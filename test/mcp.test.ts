...
// --- HEAVY burn (post-#134): deepen mcp leftover edges only — no product inventing ---
// Orthogonal to #134 (ci-config/genres/helpers/routes/parser/mcp-spec). TOKENMAXX tests-only.
...
describe('post134 mcp HEAVY deepen (after #134)', () => {
  ...
  it("post134: extra HMAC keys inventory digest", () => {
    const keys = ["mcp","backlink","Backlink Radio","station_select","now_playing","genre_filter","curator_prompt","openapi","schema_version","claw-mcp","fuzzywigg","iptv-org","post108","post134","VALID_GENRES","GENRE_MAP"];
    expect(createHash('sha256').update(keys.join('|'), 'utf8').digest('hex')).toBe("53970a44eb03463952d468097bc04f5ca89fb3cdeaa9d124a83f300a23670927");
    expect(keys).toHaveLength(16);
  });
