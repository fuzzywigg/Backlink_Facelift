...
describe('post134 genres HEAVY deepen (after #134)', () => {
  const root = genresRoot;
  const read = (rel: string) => readFileSync(join(root, rel), 'utf8');
  const sha256 = (rel: string) => createHash('sha256').update(readFileSync(join(root, rel))).digest('hex');
  const sha1 = (rel: string) => createHash('sha1').update(readFileSync(join(root, rel))).digest('hex');
  ...
  it('post134: resolveGenre maps "late night" → ambient', () => {
    expect(resolveGenre("late night")).toBe("ambient");
    expect(resolveGenre("LATE NIGHT")).toBe("ambient");
    expect(resolveGenre("  late night  ")).toBe("ambient");
  });

  it('post134: resolveGenre maps "chill" → ambient', () => {
    expect(resolveGenre("chill")).toBe("ambient");
    expect(resolveGenre("CHILL")).toBe("ambient");
    expect(resolveGenre("  chill  ")).toBe("ambient");
  });

  it('post134: resolveGenre maps "ambient" → ambient', () => {
    expect(resolveGenre("ambient")).toBe("ambient");
    expect(resolveGenre("AMBIENT")).toBe("ambient");
    expect(resolveGenre("  ambient  ")).toBe("ambient");
  });

  it('post134: resolveGenre maps "relaxing" → ambient', () => {
    expect(resolveGenre("relaxing")).toBe("ambient");
    expect(resolveGenre("RELAXING")).toBe("ambient");
    expect(resolveGenre("  relaxing  ")).toBe("ambient");
  });

  it('post134: resolveGenre maps "focus" → ambient', () => {
    expect(resolveGenre("focus")).toBe("ambient");
    expect(resolveGenre("FOCUS")).toBe("ambient");
    expect(resolveGenre("  focus  ")).toBe("ambient");
  });

  it('post134: resolveGenre maps "classical" → classical', () => {
    expect(resolveGenre("classical")).toBe("classical");
    expect(resolveGenre("CLASSICAL")).toBe("classical");
    expect(resolveGenre("  classical  ")).toBe("classical");
  });

  it('post134: resolveGenre maps "classic" → classical', () => {
    expect(resolveGenre("classic")).toBe("classical");
    expect(resolveGenre("CLASSIC")).toBe("classical");
    expect(resolveGenre("  classic  ")).toBe("classical");
  });

  it('post134: resolveGenre maps "jazz" → jazz', () => {
    expect(resolveGenre("jazz")).toBe("jazz");
    expect(resolveGenre("JAZZ")).toBe("jazz");
    expect(resolveGenre("  jazz  ")).toBe("jazz");
  });

  it('post134: resolveGenre maps "blues" → jazz', () => {
    expect(resolveGenre("blues")).toBe("jazz");
    expect(resolveGenre("BLUES")).toBe("jazz");
    expect(resolveGenre("  blues  ")).toBe("jazz");
  });

  it('post134: resolveGenre maps "pop" → pop', () => {
    expect(resolveGenre("pop")).toBe("pop");
    expect(resolveGenre("POP")).toBe("pop");
    expect(resolveGenre("  pop  ")).toBe("pop");
  });

  it('post134: resolveGenre maps "rock" → rock', () => {
    expect(resolveGenre("rock")).toBe("rock");
    expect(resolveGenre("ROCK")).toBe("rock");
    expect(resolveGenre("  rock  ")).toBe("rock");
  });

  it('post134: resolveGenre maps "metal" → rock', () => {
    expect(resolveGenre("metal")).toBe("rock");
    expect(resolveGenre("METAL")).toBe("rock");
    expect(resolveGenre("  metal  ")).toBe("rock");
  });

  it('post134: resolveGenre maps "indie" → rock', () => {
    expect(resolveGenre("indie")).toBe("rock");
    expect(resolveGenre("INDIE")).toBe("rock");
    expect(resolveGenre("  indie  ")).toBe("rock");
  });

  it('post134: resolveGenre maps "music" → music', () => {
    expect(resolveGenre("music")).toBe("music");
    expect(resolveGenre("MUSIC")).toBe("music");
    expect(resolveGenre("  music  ")).toBe("music");
  });

  it('post134: resolveGenre maps "news" → news', () => {
    expect(resolveGenre("news")).toBe("news");
    expect(resolveGenre("NEWS")).toBe("news");
    expect(resolveGenre("  news  ")).toBe("news");
  });

  it('post134: resolveGenre maps "sports" → sports', () => {
    expect(resolveGenre("sports")).toBe("sports");
    expect(resolveGenre("SPORTS")).toBe("sports");
    expect(resolveGenre("  sports  ")).toBe("sports");
  });

  it('post134: resolveGenre maps "entertainment" → entertainment', () => {
    expect(resolveGenre("entertainment")).toBe("entertainment");
    expect(resolveGenre("ENTERTAINMENT")).toBe("entertainment");
    expect(resolveGenre("  entertainment  ")).toBe("entertainment");
  });

  it('post134: resolveGenre maps "dance" → pop', () => {
    expect(resolveGenre("dance")).toBe("pop");
    expect(resolveGenre("DANCE")).toBe("pop");
    expect(resolveGenre("  dance  ")).toBe("pop");
  });

  it('post134: resolveGenre maps "electronic" → ambient', () => {
    expect(resolveGenre("electronic")).toBe("ambient");
    expect(resolveGenre("ELECTRONIC")).toBe("ambient");
    expect(resolveGenre("  electronic  ")).toBe("ambient");
  });

  it('post134: resolveGenre maps "lofi" → ambient', () => {
    expect(resolveGenre("lofi")).toBe("ambient");
    expect(resolveGenre("LOFI")).toBe("ambient");
    expect(resolveGenre("  lofi  ")).toBe("ambient");
  });

  it('post134: resolveGenre maps "lo-fi" → ambient', () => {
    expect(resolveGenre("lo-fi")).toBe("ambient");
    expect(resolveGenre("LO-FI")).toBe("ambient");
    expect(resolveGenre("  lo-fi  ")).toBe("ambient");
  });

  it('post134: resolveGenre defaults blank/undefined to music', () => {
    expect(resolveGenre()).toBe('music');
    expect(resolveGenre('')).toBe('music');
    expect(resolveGenre('   ')).toBe('music');
    expect(resolveGenre('unknown-xyz')).toBe('music');
  ...
  it('post134: VALID_GENRES length 9 ordered', () => {
    expect([...VALID_GENRES]).toEqual([
      'music', 'ambient', 'jazz', 'classical', 'pop', 'rock', 'news', 'sports', 'entertainment',
    ]);
  });

  it('post134: GENRE_MAP has 21 keys', () => {
    expect(Object.keys(GENRE_MAP)).toHaveLength(21);
  });

  it('post134: GENRE_MAP rejects __proto__ invent', () => {
    expect(Object.prototype.hasOwnProperty.call(GENRE_MAP, '__proto__')).toBe(false);
    // Plain-object map access: '__proto__' → Object.prototype ({}); do not invent hasOwn fix
    expect(resolveGenre('__proto__')).toEqual({});
  });

  it('post134: custom map override still works', () => {
    expect(resolveGenre('x', { x: 'jazz' })).toBe('jazz');
  });

  it('post134: index.ts still imports resolveGenre path', () => {
    expect(read('src/index.ts')).toMatch(/from ['"]\.\/genres['"]/);
  });

  it('post134: types.ts Env still optional GEMINI_API_KEY', () => {
    expect(read('src/types.ts')).toContain('GEMINI_API_KEY?: string');
  });

  it('post134: mega purity 40x genres.ts sha256', () => {
    const expected = "aa626817cf3bc8a707ac5adba39f811dfbc23f695e5e0cb9d070007d839d914e";
    for (let i = 0; i < 40; i++) expect(sha256('src/genres.ts')).toBe(expected);
  });

  it('post134: negative inventing fence genres', () => {
    expect(read('src/genres.ts')).not.toMatch(/\/playlist|\/now-playing|process\.env|credentials/);
  });

  
  it('post134: leftover resolveGenre trim/case/unknown/custom-map', () => {
    expect(resolveGenre('  Jazz  ')).toBe('jazz');
    expect(resolveGenre('CLASSICAL')).toBe('classical');
    expect(resolveGenre('lo-fi')).toBe('ambient');
    expect(resolveGenre('late night')).toBe('ambient');
  ...
  it('post134: leftover GENRE_MAP key vector sha256 (localeCompare)', () => {
    const keys = Object.keys(GENRE_MAP).sort((a, b) => a.localeCompare(b));
    const pairs = keys.map((k) => k + '=>' + GENRE_MAP[k]);
    expect(keys.length).toBe(21);
    expect(VALID_GENRES.length).toBe(9);
    expect(createHash('sha256').update(pairs.join('|')).digest('hex')).toBe('3363f7f62b0d2dfeecc29ae273e7a83c78d18c2f39a57523b3326b7c57b65026');
  });

  it('post134: leftover genres.ts char-window [400..700) sha256', () => {
    const slice = read('src/genres.ts').slice(400, 700);
    expect(createHash('sha256').update(slice).digest('hex')).toBe('16c259cfee1a64b3f7b2007358d528312cdf4f6440d25f86ab285ce13195abee');
  });

  it('post134: leftover digraph rank vector sha256 genres.ts (localeCompare)', () => {
    const text = read('src/genres.ts');
    ...
    expect(createHash('sha256').update(vector).digest('hex')).toBe('56347b152b93acf86c2926d269c97a2b0f1b6590b99ce963af745c41e98cf3b6');
  });
  ...
  it('post134: final inventory markers', () => {
    ...
    expect(body).toContain("describe('post126 genres HEAVY deepen (after #126)'");
    expect(body).toContain("describe('post132 genres HEAVY deepen (after #132)'");
    expect(body).toContain("describe('post134 genres HEAVY deepen (after #134)'");
