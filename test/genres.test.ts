...
// Deepens existing canonical genre validators — duplicate alias fan-in, boundary resolve, missing input.
// No product inventing — tests/CI only.

describe('overnight-seo genres HEAVY deepen (seo-score-validators after #149)', () => {
  const root = genresRoot;
  const read = (rel: string) => readFileSync(join(root, rel), 'utf8');
  const sha256 = (rel: string) => createHash('sha256').update(readFileSync(join(root, rel))).digest('hex');
  const sha1 = (rel: string) => createHash('sha1').update(readFileSync(join(root, rel))).digest('hex');
  ...
  it('overnight-seo: locks package.json sha256', () => {
    expect(sha256('package.json')).toBe('34552493f3008b58991d10e7b41ee0ecaa43bf8ba3e79d261ac2a061e6f7181c');
  });
  it('overnight-seo: locks package.json sha1', () => {
    expect(sha1('package.json')).toBe('b58d14f35b9c13bb254d5e2a51240e2918a126c5');
  });
  it('overnight-seo: locks package.json md5', () => {
    expect(md5('package.json')).toBe('63472e1fb514fb0dadb5e49a7bdbaa5f');
  });
  it('overnight-seo: locks package.json sha384', () => {
  ...
  it('overnight-seo: locks package.json sha512', () => {
  ...
  it('overnight-seo: locks package.json ripemd160', () => {
    expect(ripemd('package.json')).toBe('f3b12f3f8d6366baa145f30bfb68d5bbb06a1bad');
  });
  it('overnight-seo: locks package.json size 637', () => {
    expect(statSync(join(root, 'package.json')).size).toBe(637);
    expect(readFileSync(join(root, 'package.json')).byteLength).toBe(637);
  });
  it('overnight-seo: locks package.json utf8 635 lines 26', () => {
    expect(read('package.json')).toHaveLength(635);
    expect(read('package.json').split('\n')).toHaveLength(26);
  });
  it('overnight-seo: locks package.json nibble 451 xor 13', () => {
    const d = sha256('package.json');
    expect(nibbleSum(d)).toBe(451);
    expect(xorNibbles(d)).toBe(13);
  });
  it('overnight-seo: locks package.json pairSum 4051 rollingXor 13', () => {
    const d = sha256('package.json');
    expect(pairSum(d)).toBe(4051);
    expect(rollingXor(d)).toBe(13);
  });
  it('overnight-seo: locks package.json first/last/mid octets', () => {
    const d = sha256('package.json');
    expect(d.slice(0, 2)).toBe('34');
    expect(d.slice(-2)).toBe('1c');
    expect(d.slice(28, 36)).toBe('e0ecaa43');
  });
  it('overnight-seo: locks package.json HMAC overnight/seo-score/validators', () => {
    expect(hmacSha256('overnight', 'package.json')).toBe('45c3592147b357a6bb77a8190520c5a786cff8446d2d7f4c96b2535e9c907990');
    expect(hmacSha256('seo-score', 'package.json')).toBe('4727f60228d8241fe1c3af16139b7e82b0d8e04f2a27f22061978e7f6f95e581');
    expect(hmacSha256('validators', 'package.json')).toBe('1c8ac90fb1d5ffb2a954906e0d5293453f358ea1c24af9b70a76057dd50b786f');
  });
  it('overnight-seo: locks package.json HMAC TOKENMAXX/HEAVY/after-#149', () => {
  ...
  it('overnight-seo: locks package.json HMAC leftover/no-product-invent/boundary-scores', () => {
    ...
    expect(hmacSha256('boundary-scores', 'package.json')).toBe('c94af1a4f3a60bbcf6d7bfe3323bdb8b66b522224e9840c76f88a3d640dba56b');
  });
  it('overnight-seo: locks package.json HMAC missing-meta/duplicate-canonical/zero-link', () => {
    expect(hmacSha256('missing-meta', 'package.json')).toBe('86c1bf1175f1b78d78dcf4cf23916138a08d3950dca2154cfae9ff36968a29d7');
    expect(hmacSha256('duplicate-canonical', 'package.json')).toBe('301cc93ec9b3fdbe4b30c4ecc124b70eb608d66346df111986ab966b7fd35e09');
    expect(hmacSha256('zero-link', 'package.json')).toBe('d555a32d7134c35b9ace8d6492b877c3cfbcbd5c2772f8d7e5d4fde8176c16a5');
  });
  it('overnight-seo: locks package.json HMAC canonical-stub/seo-score-validators', () => {
    expect(hmacSha256('canonical-stub', 'package.json')).toBe('31b405a8f43c1c8972fc71461ac66377b5af3c835b1d6ad31efec04df993322b');
    expect(hmacSha256('seo-score-validators', 'package.json')).toBe('220b83529719ae040b56f7a12a7a029725503bef37a85369311234ad6e10497b');
  });
  it('overnight-seo: locks package.json spaces 106', () => {
    expect((read('package.json').match(/ /g) ?? []).length).toBe(106);
  });
  it('overnight-seo: locks package.json reversed sha256', () => {
  ...
  it('overnight-seo: locks package.json size*lines 16562', () => {
    expect(statSync(join(root, 'package.json')).size * read('package.json').split('\n').length).toBe(16562);
  });
  it('overnight-seo: locks package.json char-class tabs/nl/comma/colon/quotes', () => {
  ...
  it('overnight-seo: locks package.json HMAC-SHA1/MD5 key overnight-seo', () => {
    expect(createHmac('sha1', 'overnight-seo').update(readFileSync(join(root, 'package.json'))).digest('hex')).toBe('f1979ec29b1bf4085655c7903deff8089bd0cc1a');
    expect(createHmac('md5', 'overnight-seo').update(readFileSync(join(root, 'package.json'))).digest('hex')).toBe('4c3034462677290c2c7dfdb901eb5719');
  });
  it('overnight-seo: duplicate-canonical fan-in → ambient (8 aliases)', () => {
    for (const a of ["late night","chill","ambient","relaxing","focus","electronic","lofi","lo-fi"]) {
      expect(resolveGenre(a)).toBe('ambient');
      expect(resolveGenre(a.toUpperCase())).toBe('ambient');
      expect(resolveGenre('  ' + a + '  ')).toBe('ambient');
    }
  });
  it('overnight-seo: duplicate-canonical fan-in → classical (2 aliases)', () => {
    for (const a of ["classical","classic"]) {
      expect(resolveGenre(a)).toBe('classical');
      expect(resolveGenre(a.toUpperCase())).toBe('classical');
      expect(resolveGenre('  ' + a + '  ')).toBe('classical');
    }
  });
  it('overnight-seo: duplicate-canonical fan-in → jazz (2 aliases)', () => {
    for (const a of ["jazz","blues"]) {
      expect(resolveGenre(a)).toBe('jazz');
      expect(resolveGenre(a.toUpperCase())).toBe('jazz');
      expect(resolveGenre('  ' + a + '  ')).toBe('jazz');
    }
  });
  it('overnight-seo: duplicate-canonical fan-in → rock (3 aliases)', () => {
    for (const a of ["rock","metal","indie"]) {
      expect(resolveGenre(a)).toBe('rock');
      expect(resolveGenre(a.toUpperCase())).toBe('rock');
      expect(resolveGenre('  ' + a + '  ')).toBe('rock');
    }
  });
  it('overnight-seo: duplicate-canonical fan-in → pop (2 aliases)', () => {
    for (const a of ["pop","dance"]) {
      expect(resolveGenre(a)).toBe('pop');
      expect(resolveGenre(a.toUpperCase())).toBe('pop');
      expect(resolveGenre('  ' + a + '  ')).toBe('pop');
    }
  });
  it('overnight-seo: duplicate-canonical fan-in → music (1 aliases)', () => {
    for (const a of ["music"]) {
      expect(resolveGenre(a)).toBe('music');
      expect(resolveGenre(a.toUpperCase())).toBe('music');
      expect(resolveGenre('  ' + a + '  ')).toBe('music');
    }
  });
  it('overnight-seo: duplicate-canonical fan-in → news (1 aliases)', () => {
    for (const a of ["news"]) {
      expect(resolveGenre(a)).toBe('news');
      expect(resolveGenre(a.toUpperCase())).toBe('news');
      expect(resolveGenre('  ' + a + '  ')).toBe('news');
    }
  });
  it('overnight-seo: duplicate-canonical fan-in → sports (1 aliases)', () => {
    for (const a of ["sports"]) {
      expect(resolveGenre(a)).toBe('sports');
      expect(resolveGenre(a.toUpperCase())).toBe('sports');
      expect(resolveGenre('  ' + a + '  ')).toBe('sports');
    }
  });
  it('overnight-seo: duplicate-canonical fan-in → entertainment (1 aliases)', () => {
    for (const a of ["entertainment"]) {
      expect(resolveGenre(a)).toBe('entertainment');
      expect(resolveGenre(a.toUpperCase())).toBe('entertainment');
      expect(resolveGenre('  ' + a + '  ')).toBe('entertainment');
    }
  });
  it('overnight-seo: boundary-scores missing/blank/unknown default music', () => {
    expect(resolveGenre()).toBe('music');
    expect(resolveGenre('')).toBe('music');
    expect(resolveGenre('   ')).toBe('music');
    expect(resolveGenre('unknown-seo-xyz')).toBe('music');
    expect(resolveGenre('SEO_SCORE_FAKE')).toBe('music');
  });
  it('overnight-seo: missing-meta VALID_GENRES still accepted as identity', () => {
    for (const g of VALID_GENRES) expect(resolveGenre(g)).toBe(g);
  });
  it('overnight-seo: GENRE_MAP key count 21 lock', () => {
    expect(Object.keys(GENRE_MAP)).toHaveLength(21);
  });
  it('overnight-seo: VALID_GENRES length 9 ordered', () => {
    expect([...VALID_GENRES]).toEqual(['music','ambient','jazz','classical','pop','rock','news','sports','entertainment']);
  });
  it('overnight-seo: zero-link custom empty map falls through to music', () => {
    expect(resolveGenre('jazz', {})).toBe('jazz');
    expect(resolveGenre('not-a-genre', {})).toBe('music');
  });
  it('overnight-seo: GENRE_MAP rejects __proto__ invent', () => {
    expect(Object.prototype.hasOwnProperty.call(GENRE_MAP, '__proto__')).toBe(false);
    expect(resolveGenre('__proto__')).toEqual({});
  });
  it('overnight-seo: resolveGenre maps "late night" → ambient', () => {
    expect(resolveGenre("late night")).toBe('ambient');
    expect(resolveGenre("late night".toUpperCase())).toBe('ambient');
    expect(resolveGenre('  ' + "late night" + '  ')).toBe('ambient');
  });
  it('overnight-seo: resolveGenre maps "chill" → ambient', () => {
    expect(resolveGenre("chill")).toBe('ambient');
    expect(resolveGenre("chill".toUpperCase())).toBe('ambient');
    expect(resolveGenre('  ' + "chill" + '  ')).toBe('ambient');
  });
  it('overnight-seo: resolveGenre maps "ambient" → ambient', () => {
    expect(resolveGenre("ambient")).toBe('ambient');
    expect(resolveGenre("ambient".toUpperCase())).toBe('ambient');
    expect(resolveGenre('  ' + "ambient" + '  ')).toBe('ambient');
  });
  it('overnight-seo: resolveGenre maps "relaxing" → ambient', () => {
    expect(resolveGenre("relaxing")).toBe('ambient');
    expect(resolveGenre("relaxing".toUpperCase())).toBe('ambient');
    expect(resolveGenre('  ' + "relaxing" + '  ')).toBe('ambient');
  });
  it('overnight-seo: resolveGenre maps "focus" → ambient', () => {
    expect(resolveGenre("focus")).toBe('ambient');
    expect(resolveGenre("focus".toUpperCase())).toBe('ambient');
    expect(resolveGenre('  ' + "focus" + '  ')).toBe('ambient');
  });
  it('overnight-seo: resolveGenre maps "classical" → classical', () => {
    expect(resolveGenre("classical")).toBe('classical');
    expect(resolveGenre("classical".toUpperCase())).toBe('classical');
    expect(resolveGenre('  ' + "classical" + '  ')).toBe('classical');
  });
  it('overnight-seo: resolveGenre maps "classic" → classical', () => {
    expect(resolveGenre("classic")).toBe('classical');
    expect(resolveGenre("classic".toUpperCase())).toBe('classical');
    expect(resolveGenre('  ' + "classic" + '  ')).toBe('classical');
  });
  it('overnight-seo: resolveGenre maps "jazz" → jazz', () => {
    expect(resolveGenre("jazz")).toBe('jazz');
    expect(resolveGenre("jazz".toUpperCase())).toBe('jazz');
    expect(resolveGenre('  ' + "jazz" + '  ')).toBe('jazz');
  });
  it('overnight-seo: resolveGenre maps "blues" → jazz', () => {
    expect(resolveGenre("blues")).toBe('jazz');
    expect(resolveGenre("blues".toUpperCase())).toBe('jazz');
    expect(resolveGenre('  ' + "blues" + '  ')).toBe('jazz');
  });
  it('overnight-seo: resolveGenre maps "pop" → pop', () => {
    expect(resolveGenre("pop")).toBe('pop');
    expect(resolveGenre("pop".toUpperCase())).toBe('pop');
    expect(resolveGenre('  ' + "pop" + '  ')).toBe('pop');
  });
  it('overnight-seo: resolveGenre maps "rock" → rock', () => {
    expect(resolveGenre("rock")).toBe('rock');
    expect(resolveGenre("rock".toUpperCase())).toBe('rock');
    expect(resolveGenre('  ' + "rock" + '  ')).toBe('rock');
  });
  it('overnight-seo: resolveGenre maps "metal" → rock', () => {
    expect(resolveGenre("metal")).toBe('rock');
    expect(resolveGenre("metal".toUpperCase())).toBe('rock');
    expect(resolveGenre('  ' + "metal" + '  ')).toBe('rock');
  });
  it('overnight-seo: resolveGenre maps "indie" → rock', () => {
    expect(resolveGenre("indie")).toBe('rock');
    expect(resolveGenre("indie".toUpperCase())).toBe('rock');
    expect(resolveGenre('  ' + "indie" + '  ')).toBe('rock');
  });
  it('overnight-seo: resolveGenre maps "music" → music', () => {
    expect(resolveGenre("music")).toBe('music');
    expect(resolveGenre("music".toUpperCase())).toBe('music');
    expect(resolveGenre('  ' + "music" + '  ')).toBe('music');
  });
  it('overnight-seo: resolveGenre maps "news" → news', () => {
    expect(resolveGenre("news")).toBe('news');
    expect(resolveGenre("news".toUpperCase())).toBe('news');
    expect(resolveGenre('  ' + "news" + '  ')).toBe('news');
  });
  it('overnight-seo: resolveGenre maps "sports" → sports', () => {
    expect(resolveGenre("sports")).toBe('sports');
    expect(resolveGenre("sports".toUpperCase())).toBe('sports');
    expect(resolveGenre('  ' + "sports" + '  ')).toBe('sports');
  });
  it('overnight-seo: resolveGenre maps "entertainment" → entertainment', () => {
    expect(resolveGenre("entertainment")).toBe('entertainment');
    expect(resolveGenre("entertainment".toUpperCase())).toBe('entertainment');
    expect(resolveGenre('  ' + "entertainment" + '  ')).toBe('entertainment');
  });
  it('overnight-seo: resolveGenre maps "dance" → pop', () => {
    expect(resolveGenre("dance")).toBe('pop');
    expect(resolveGenre("dance".toUpperCase())).toBe('pop');
    expect(resolveGenre('  ' + "dance" + '  ')).toBe('pop');
  });
  it('overnight-seo: resolveGenre maps "electronic" → ambient', () => {
    expect(resolveGenre("electronic")).toBe('ambient');
    expect(resolveGenre("electronic".toUpperCase())).toBe('ambient');
    expect(resolveGenre('  ' + "electronic" + '  ')).toBe('ambient');
  });
  it('overnight-seo: resolveGenre maps "lofi" → ambient', () => {
    expect(resolveGenre("lofi")).toBe('ambient');
    expect(resolveGenre("lofi".toUpperCase())).toBe('ambient');
    expect(resolveGenre('  ' + "lofi" + '  ')).toBe('ambient');
  });
  it('overnight-seo: resolveGenre maps "lo-fi" → ambient', () => {
    expect(resolveGenre("lo-fi")).toBe('ambient');
    expect(resolveGenre("lo-fi".toUpperCase())).toBe('ambient');
    expect(resolveGenre('  ' + "lo-fi" + '  ')).toBe('ambient');
  });
  it('overnight-seo: negative inventing fence genres', () => {
    expect(read('src/genres.ts')).not.toMatch(/\/playlist|\/now-playing|process\.env|credentials|seoScore|pageRank/);
  });
  it('overnight-seo: mega purity 40x genres.ts sha256', () => {
    const expected = "aa626817cf3bc8a707ac5adba39f811dfbc23f695e5e0cb9d070007d839d914e";
    for (let i = 0; i < 40; i++) expect(sha256('src/genres.ts')).toBe(expected);
  });
  ...
  it('overnight-seo: final inventory markers', () => {
    const body = read('test/genres.test.ts');
    expect(body).toContain("describe('post146 genres HEAVY deepen (after #146)'");
    expect(body).toContain("describe('overnight-seo genres HEAVY deepen (seo-score-validators after #149)'");
    expect((body.match(/it\('overnight-seo:/g) ?? []).length).toBeGreaterThan(60);
  });
});

describe('overnight-seo genres extras HEAVY deepen (seo-score-validators leftover)', () => {
  const root = genresRoot;
  const read = (rel: string) => readFileSync(join(root, rel), 'utf8');
  const sha256 = (rel: string) => createHash('sha256').update(readFileSync(join(root, rel))).digest('hex');
  const sha1 = (rel: string) => createHash('sha1').update(readFileSync(join(root, rel))).digest('hex');
  ...
  it('overnight-seo-extras: final inventory markers', () => {
    const body = read('test/genres.test.ts');
    expect(body).toContain("describe('overnight-seo genres HEAVY deepen (seo-score-validators after #149)')");
    expect(body).toContain("describe('overnight-seo genres extras HEAVY deepen (seo-score-validators leftover)')");
