...
describe('post144 genres HEAVY deepen (after #144)', () => {
  const root = genresRoot;
  const read = (rel: string) => readFileSync(join(root, rel), 'utf8');
  const sha256 = (rel: string) => createHash('sha256').update(readFileSync(join(root, rel))).digest('hex');
  const sha1 = (rel: string) => createHash('sha1').update(readFileSync(join(root, rel))).digest('hex');
  ...
  it('post144: resolveGenre maps "dance" → pop', () => {
    expect(resolveGenre('dance')).toBe('pop');
    expect(resolveGenre('DANCE')).toBe('pop');
    expect(resolveGenre('  dance  ')).toBe('pop');
  });

  it('post144: resolveGenre maps "electronic" → ambient', () => {
    expect(resolveGenre('electronic')).toBe('ambient');
    expect(resolveGenre('ELECTRONIC')).toBe('ambient');
  });

  it('post144: resolveGenre maps "lofi"/"lo-fi" → ambient', () => {
    expect(resolveGenre('lofi')).toBe('ambient');
    expect(resolveGenre('lo-fi')).toBe('ambient');
  });

  it('post144: resolveGenre defaults blank/undefined to music', () => {
    expect(resolveGenre()).toBe('music');
    expect(resolveGenre('')).toBe('music');
    expect(resolveGenre('   ')).toBe('music');
    expect(resolveGenre('unknown-xyz')).toBe('music');
  ...
  it('post144: VALID_GENRES length 9 ordered', () => {
    expect([...VALID_GENRES]).toEqual([
      'music', 'ambient', 'jazz', 'classical', 'pop', 'rock', 'news', 'sports', 'entertainment',
    ]);
  });

  it('post144: GENRE_MAP has 21 keys', () => {
    expect(Object.keys(GENRE_MAP)).toHaveLength(21);
  });

  it('post144: GENRE_MAP rejects __proto__ invent', () => {
    expect(Object.prototype.hasOwnProperty.call(GENRE_MAP, '__proto__')).toBe(false);
    expect(resolveGenre('__proto__')).toEqual({});
  });

  it('post144: custom map override still works', () => {
    expect(resolveGenre('x', { x: 'jazz' })).toBe('jazz');
  });
  ...
  it('post144: final inventory markers', () => {
    const body = read('test/genres.test.ts');
    expect(body).toContain("describe('post144 genres HEAVY deepen (after #144)'");
    expect((body.match(/it\('post144:/g) ?? []).length).toBeGreaterThan(60);
  });
});


describe('post144 genres extras HEAVY deepen (after #144 leftover slice)', () => {
  const root = genresRoot;
  const read = (rel: string) => readFileSync(join(root, rel), 'utf8');
  const sha256 = (rel: string) => createHash('sha256').update(readFileSync(join(root, rel))).digest('hex');
  const sha1 = (rel: string) => createHash('sha1').update(readFileSync(join(root, rel))).digest('hex');
  ...
  it('post144-extras: inventory digest', () => {
    const keys = ["post144","leftover","TOKENMAXX","HEAVY","after-#144","no-product-invent","slice-diff","no-src-change","manifest-lock","HITL","genres","extras","VALID_GENRES","GENRE_MAP","CATALOG_CACHE","post141"];
    expect(createHash('sha256').update(keys.join('|'), 'utf8').digest('hex')).toBe('901cfc3504d1d9a281cf2b066f1530d6a3fdfc6c7828fbc132ec6fc964d8ffb7');
  ...
  it('post144-extras: final markers', () => {
    const body = read('test/genres.test.ts');
    expect(body).toContain("describe('post144 genres HEAVY deepen (after #144)'");
    expect(body).toContain("describe('post144 genres extras HEAVY deepen (after #144 leftover slice)'");
