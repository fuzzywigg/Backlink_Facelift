...
describe('post144 mcp-spec-contract HEAVY deepen (after #144)', () => {
  const read = (rel: string) => readFileSync(join(root, rel), 'utf8');
  const sha256 = (rel: string) => createHash('sha256').update(readFileSync(join(root, rel))).digest('hex');
  const sha1 = (rel: string) => createHash('sha1').update(readFileSync(join(root, rel))).digest('hex');
  const md5 = (rel: string) => createHash('md5').update(readFileSync(join(root, rel))).digest('hex');
  ...
  it('post144: MCP_MANIFEST tool count 4', () => {
    expect(MCP_MANIFEST.tools).toHaveLength(4);
  });

  it('post144: MCP_MANIFEST schema_version v1', () => {
    expect(MCP_MANIFEST.schema_version).toBe('v1');
  });

  it('post144: MCP_MANIFEST name_for_model backlink', () => {
    expect(MCP_MANIFEST.name_for_model).toBe('backlink');
  });

  it('post144: MCP_MANIFEST name_for_human Backlink Radio', () => {
    expect(MCP_MANIFEST.name_for_human).toBe('Backlink Radio');
  });

  it('post144: MCP_MANIFEST auth none', () => {
    expect(MCP_MANIFEST.auth).toEqual({ type: 'none' });
  });

  it('post144: MCP_MANIFEST api openapi', () => {
    expect(MCP_MANIFEST.api).toEqual({ type: 'openapi', url: '/openapi.json' });
  });

  it('post144: tools station_select/now_playing/genre_filter/curator_prompt', () => {
    const names = MCP_MANIFEST.tools.map((t) => t.name);
    expect(names).toEqual(['station_select', 'now_playing', 'genre_filter', 'curator_prompt']);
    for (const n of names) expect(n.startsWith('claw_')).toBe(false);
  });

  it('post144: docs tools backlink_* distinct from claw-mcp', () => {
    const spec = read('docs/mcp-spec.md');
    expect(spec).toContain('backlink_curate');
    expect(spec).toContain('backlink_genres');
    expect(spec).not.toContain('claw_station_select');
  });

  it('post144: docs base URL lock (no DNS invent)', () => {
    expect(read('docs/mcp-spec.md')).toMatch(/Base URL:\s*`https:\/\/backlink\.fuzzywigg\.com`/);
  });
  ...
  it('post144: final inventory markers', () => {
    const body = read('test/mcp-spec-contract.test.ts');
    expect(body).toContain("describe('post144 mcp-spec-contract HEAVY deepen (after #144)'");
    expect((body.match(/it\('post144:/g) ?? []).length).toBeGreaterThan(60);
  });
});


describe('post144 mcp-spec-contract extras HEAVY deepen (after #144 leftover slice)', () => {
  const read = (rel: string) => readFileSync(join(root, rel), 'utf8');
  const sha256 = (rel: string) => createHash('sha256').update(readFileSync(join(root, rel))).digest('hex');
  const sha1 = (rel: string) => createHash('sha1').update(readFileSync(join(root, rel))).digest('hex');
  const md5 = (rel: string) => createHash('md5').update(readFileSync(join(root, rel))).digest('hex');
  ...
  it('post144-extras: inventory digest', () => {
    const keys = ["post144","leftover","TOKENMAXX","HEAVY","after-#144","no-product-invent","slice-diff","no-src-change","manifest-lock","HITL","mcp-spec-contract","extras","VALID_GENRES","GENRE_MAP","CATALOG_CACHE","post141"];
    expect(createHash('sha256').update(keys.join('|'), 'utf8').digest('hex')).toBe('ea203bc27f7aa53c259ce2d7f9826029ee28c99675b5d2d5514157e475e63e58');
  ...
  it('post144-extras: final markers', () => {
    const body = read('test/mcp-spec-contract.test.ts');
    expect(body).toContain("describe('post144 mcp-spec-contract HEAVY deepen (after #144)'");
    expect(body).toContain("describe('post144 mcp-spec-contract extras HEAVY deepen (after #144 leftover slice)'");
